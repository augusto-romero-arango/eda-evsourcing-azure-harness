# MEF-ADR-0027: Enrutamiento multi-destinatario de un evento por correlation filter de igualdad

- **Fecha**: 2026-07-15
- **Estado**: aceptado
- **Aplica a**: doctrina de mensajeria del marco; complementa MEF-ADR-0001 (no lo reemplaza) con una excepcion gobernada al rechazo de filtros SQL; cross-referencia MEF-ADR-0001, MEF-ADR-0024, MEF-ADR-0025 y MEF-ADR-0026 (advertencia sobre no reusar `SessionId`). Gobierno futuro de `infra-base-scaffolder`/`infra-writer`/`infra-reviewer` (infraestructura, diferido) e `implementer` (guia del productor, diferido).

## Contexto

MEF-ADR-0001 fija topic-por-tipo-de-evento como topologia por defecto y descarta explicitamente los
...
(`docs/adr/mef-adr-0001-service-bus-topics-por-evento.md:15-26,57-61`). Ese rechazo razona sobre un eje
concreto: **varios tipos de evento** conviviendo en un topic de dominio, filtrados por **tipo** con
SQL.

Existe un eje distinto que MEF-ADR-0001 no contemplo: un **unico** evento publico (`IPublicEvent`)
...
MEF-ADR-0024: el evento publico, plano y portable, ya viaja con propiedades en el sobre (application
...
visible en Terraform, igual que exige MEF-ADR-0001: el filtro de cada destinatario se lee en su
subscription, sin cruzar a codigo de aplicacion.

MEF-ADR-0026 (issue #269) fijo el patron a seguir cuando un eje nuevo de la topologia de mensajeria no
encaja en MEF-ADR-0001: doctrina en un ADR nuevo, con una cross-referencia minima de vuelta a MEF-ADR-0001
que acota su absoluto sin reabrirlo, en vez de reformar MEF-ADR-0001 en profundidad. Este ADR sigue el
mismo patron, con la advertencia adicional de MEF-ADR-0026: la clave de enrutamiento de este ADR **no
debe conflacionarse** con `SessionId`, reservado para la serializacion de fan-in de MEF-ADR-0026 -- son
mecanismos distintos aunque ambos operen sobre application properties.

## Decision

### 1. Un eje distinto al que MEF-ADR-0001 rechazo: filtrar por destinatario, no por tipo

MEF-ADR-0001 rechaza filtros SQL para el eje "varios tipos de evento en un topic de dominio, filtrar
...
subscriptions -- MEF-ADR-0001 no cambia: el productor sigue publicando unicamente a topics. Lo nuevo es
...
### 3. Solo igualdad -- el rechazo de filtros SQL de MEF-ADR-0001 no se toca
...
correlation filter: sigue cayendo, sin excepcion, bajo el rechazo de MEF-ADR-0001.

### 4. Construccion exclusiva del lado consumidor; el productor no cambia de contrato

Igual que el queue con sesion de MEF-ADR-0026, el correlation filter es una construccion del **lado
...
cambio de contrato (MEF-ADR-0001/MEF-ADR-0024); su unica obligacion nueva es **estampar la clave de
...
MEF-ADR-0026 reserva `SessionId` (el `groupId` de `IPrivateEventSender`) para la serializacion de
...
contradice el principio de MEF-ADR-0001 de que la topologia completa (quien consume que) se lee en
...
reabriria el rechazo de MEF-ADR-0001 sin beneficio real.
...
**Descartada**: rompe la invariante de MEF-ADR-0001 de que cada tipo de evento tiene **un** topic, y
...
destinatarios sin tocar al productor, igual que ya vale para el caso simple de MEF-ADR-0001.

## Consecuencias

### Positivas

- **Cierra un vacio real de la doctrina**: MEF-ADR-0001 no tenia una respuesta declarativa para "un
...
- **No reabre el rechazo de filtros SQL**: el mecanismo sigue siendo estrictamente de igualdad;
  MEF-ADR-0001 mantiene su rechazo de SQL intacto para cualquier caso que exija mas que igualdad.
...
- **Sin ambiguedad con MEF-ADR-0026**: se documenta explicitamente que la clave de enrutamiento y
...
- MEF-ADR-0001 (Service Bus: un topic por tipo de evento): este ADR acota, sin reemplazar, el rechazo
  de filtros SQL -- fija la excepcion gobernada del correlation filter de igualdad para el eje "un
  evento, N destinatarios", distinto del eje "varios tipos de evento" que MEF-ADR-0001 razono.
- MEF-ADR-0024 (modelo de eventos de bus, privado/publico): el evento publico cruza el backbone
...
- MEF-ADR-0025 (custodia de secretos): la clave de enrutamiento no es un secreto ni una key; se
  documenta en claro, sin custodiarse en Key Vault.
- MEF-ADR-0026 (colas de Service Bus con sesion para fan-in): precedente del patron ADR-nuevo +
  cross-ref minima hacia MEF-ADR-0001; advierte no conflacionar la clave de enrutamiento de este ADR
