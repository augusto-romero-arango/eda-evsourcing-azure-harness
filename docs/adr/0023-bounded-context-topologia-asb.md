...
- **Aislamiento estructural**: la topologia de dos namespaces aísla el trafico interno del publico sin dependencia de convencion de naming o configuracion de RBAC fino dentro de un namespace compartido.
...
- **[3]** "Azure built-in roles — Azure Service Bus Data Sender / Data Receiver" — definicion oficial de los roles integrados `4f6d3b9b-027b-4f4c-9142-0e5a2a2247e0` (Data Sender) y `090c5cfd-751d-490a-894a-3ce6f1109419` (Data Receiver). https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/integration
- ADR-0001 (un topic por tipo de evento): ADR-0023 anade el dimension faltante — en que namespace vive ese topic (interno vs integracion) segun el alcance del evento.
