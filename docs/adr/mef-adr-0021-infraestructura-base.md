...
- 2026-08-17: enmendado (issue #652) para cerrar el wiring de `always_on` prometido en la nota sobre el output del modulo `service-plan`: se anade `always_on` a los inputs principales del modulo `function-app` en la tabla de los 8 modulos, y esa nota deja constancia de que el modulo `function-app` lo aplica en `site_config.always_on`. Sin este cambio el output del `service-plan` quedaba huerfano: el valor pedido por el usuario no llegaba a la Function App real y prevalecia el default `false` del provider `azurerm`.
