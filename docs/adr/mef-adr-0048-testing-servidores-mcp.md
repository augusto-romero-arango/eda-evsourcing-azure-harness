...
- **Fecha**: 2026-08-30
- **Estado**: aceptado
- **Aplica a**: doctrina general del marco para probar todo servidor MCP (Model Context Protocol) que un Bounded Context expone bajo MEF-ADR-0047 -- la piramide de niveles de test, las verificaciones canonicas del nivel e2e (incluida, desde la enmienda del issue #841, la cobertura obligatoria de cada parametro de forma fecha o identificador con un valor valido representativo), la obligacion de extender esa suite ante toda tool nueva o modificada, los endpoints de gate propios de una app MCP y la obtencion de su credencial en CI. Aplicable a cualquier Bounded Context de cualquier consumidor del marco; el cuerpo normativo no referencia ningun proyecto, archivo ni issue de un consumidor concreto (ver seccion "Alcance"). Extiende MEF-ADR-0013 (smoke tests contra dev), MEF-ADR-0022 (autenticacion de CI por OIDC) y MEF-ADR-0031 (readiness gate por SHA) al dominio de servidores MCP; cita MEF-ADR-0016 (naming de metodos de test) y MEF-ADR-0018 (Rule of Three) como doctrina ya vigente que este ADR no reabre. Cross-referencia la restriccion de coercion de argumentos string que MEF-ADR-0047 decision 1 fija (enmienda del issue #841 en ambos ADRs).
...
- **Cuando extraer codigo duplicado entre servidores MCP del mismo BC.** MEF-ADR-0018 (Rule of Three) ya fija el criterio general; este ADR solo senala, en la seccion 4, un caso concreto donde ese criterio aplica sin fijar una regla nueva.
...
**Corolario del estilo de declaracion.** La extension admite declarar las propiedades de una tool de tres formas (atributo `[McpToolProperty]`, POCO de argumentos, o el builder `ConfigureMcpTool(...).WithProperty(...)` en `Program.cs`); un test de nivel 2 que refleja atributos solo ve las dos primeras. Un servidor que declare propiedades por el builder deja ese pinneo estructuralmente fuera del alcance del nivel 2, y toda la carga de verificar `required` recae en la verificacion 2 del nivel 3 (seccion 2) -- razon adicional para preferir la declaracion por atributo o POCO.

**Limite estructural que fija el nivel 2 en composicion, nunca en runtime.** El registro que sirve `tools/list` en runtime (`DefaultToolRegistry` de la extension MCP de Azure Functions) vive en el paquete del **host**, no en el ensamblado del **worker** que un test de composicion carga y refleja -- un unit test no puede instanciarlo ni invocarlo. Por eso el nivel 2 verifica la **declaracion** (los atributos y metadata que el worker compilado expone), no la **respuesta servida**; verificar la respuesta servida en runtime es exclusivamente el trabajo del nivel 3.
...
2. **`tools/list` vivo como contrato pinneado.** El catalogo exacto de tools que el servidor publica en runtime, el `required` de cada `inputSchema`, y el `readOnlyHint` (u otro `ToolAnnotations`) presente en `_meta` -- pinneados contra el diseno esperado, no solo verificados como "no vacio". Es la unica verificacion posible del contrato realmente servido (ver el limite estructural de la seccion 1): si el nivel 2 pinnea la declaracion y este nivel no pinnea la respuesta servida, un desalineamiento entre lo que el worker declara y lo que el host efectivamente publica pasa inadvertido.
3. **Una tool call de lectura contra datos reales, con cada parametro de forma fecha o identificador ejercitado con un valor valido representativo.** Invoca una tool real con argumentos que produzcan una respuesta con datos genuinos del BC -- recorre la cadena completa cliente -> host -> worker -> `HttpClient` tipado -> Function App del BC -> dato real, no un stub. Cuando el `inputSchema` de la tool declara un parametro de forma fecha (p. ej. `yyyy-MM-dd`) o identificador (un GUID en el formato que el dominio usa), esta tool call pasa, para **cada** parametro de esa forma, un valor valido y representativo del formato que la tool promete, y el assert confirma que el resultado es el dato de negocio esperado -- nunca el mensaje de validacion de la verificacion 4. Es el unico camino de la suite que ejercita la restriccion conocida de la extension MCP (MEF-ADR-0047 decision 1: coercion de todo string con forma de fecha o GUID antes de que la tool lo reciba). **Ni la tool call que omite ese parametro como opcional, ni el error path de la verificacion 4 sustituyen esta cobertura**: un valor invalido que no parece fecha (p. ej. `"2026-99-99"`) no dispara la coercion y llega intacto, asi que el error path prueba la validacion de la tool, nunca la ruta que la coercion rompe.
...
- **Proscrito**: `mcp_extension` como GitHub secret. Copiar la key a un secret del repo introduce exactamente lo que el mecanismo runtime evita -- una copia persistida que exige rotacion manual coordinada si la key del host cambia, y una superficie de fuga adicional (el secret sobrevive mas alla de la ejecucion del job que lo necesito). La propia documentacion de Azure Functions lo advierte para la salida de estos comandos: *"Because the output contains sensitive information, don't persist the output or secure any persisted file outputs"* (documentacion oficial de access keys de Azure Functions, citada en "Referencias").

### 5. Criterio de concurrencia: cuando una suite MCP entra al grupo compartido (CA-5)

MEF-ADR-0013 corre los smoke tests de cada dominio secuencialmente **dentro del ensamblado** (`[assembly: DisableParallelization]`) porque comparten la suscripcion `smoke-tests` de Service Bus: la purga o el `CompleteMessageAsync` de una suite puede robar el mensaje que otra espera. Serializar los smoke **entre workflows** es un mecanismo distinto y opcional -- un grupo `concurrency` de GitHub Actions que **las plantillas del marco no declaran**, y que un consumidor puede declarar por su cuenta (el consumidor de origen lo hace con `smoke-tests-dev`; MEF-ADR-0031 seccion 4) --, pero su razon de ser es la misma: el recurso compartido del bus.

- **Una suite MCP de solo lectura (piramide de las secciones 1-2 sin ninguna tool de escritura) no entra a ese grupo de concurrencia compartido, alli donde el consumidor lo declare.** La razon de ser del grupo -- el robo de mensajes de Service Bus entre suites -- no aplica: un servidor MCP de solo lectura no publica ni consume del bus, asi que no compite por la suscripcion compartida con ninguna otra suite.
- **Una suite MCP que gana una fase de comandos/siembra con efectos secundarios reales (Service Bus, persistencia) si entra al grupo**, exactamente por el mismo motivo que cualquier smoke test de dominio: en cuanto una tool produce un efecto secundario compartido, el riesgo de robo de mensaje que motiva el grupo vuelve a aplicar. **Al entrar hereda la trampa que MEF-ADR-0031 seccion 4 documenta**: si esa suite espera a un run ajeno del mismo commit, debe esperar el **job** `deploy` de ese run y nunca el run completo -- el run ajeno pide el mismo grupo y no puede avanzar hasta que el que espera lo libere, asi que esperarlo entero es un deadlock, no una carrera.

### 6. Extension obligatoria de la suite ante una tool nueva o modificada
...
### Alt 4: incluir toda suite MCP en el grupo de concurrencia compartido de smoke tests por defecto
...
- MEF-ADR-0013 (smoke tests contra entorno dev): ADR hermano que este documento extiende al dominio de servidores MCP; su control de cambios se enmienda con una linea que remite a este ADR.
...
- Microsoft Learn, "MCP tool trigger for Azure Functions" (C#) -- fuente de los atributos `McpToolTriggerAttribute`/`McpToolPropertyAttribute` que el nivel 2 refleja (seccion 1), del esquema `ToolProperty` (`isRequired`, `propertyType`) que la verificacion 2 de la seccion 2 pinnea contra el `inputSchema` publicado, y de las tres formas admitidas de declarar propiedades -- atributo, POCO de argumentos y `ConfigureMcpTool(...).WithProperty(...)` en `Program.cs` -- de donde sale el corolario de la seccion 1 sobre que ve y que no ve la reflexion. https://learn.microsoft.com/azure/azure-functions/functions-bindings-mcp-tool-trigger
- Microsoft Learn, "Work with access keys in Azure Functions" -- fuente del tipo de key **System** (*"Only specific extensions can create system keys. You can't explicitly set their values"*), del comando `az functionapp keys list` y de la advertencia que la seccion 4 cita para proscribir la copia persistida de la key. https://learn.microsoft.com/azure/azure-functions/function-keys-how-to
- Microsoft Learn, "az functionapp keys list" (referencia de Azure CLI) -- el comando concreto que la seccion 4 ejecuta en runtime. Su equivalente en el plano de control es la operacion ARM *List Host Keys*; la accion RBAC correspondiente (`Microsoft.Web/sites/host/listkeys/action`) no figura entre las `NotActions` del rol `Contributor` que el SP de CI ya tiene por MEF-ADR-0022 (ver la referencia [9] de ese ADR), asi que la operacion no exige rol adicional. https://learn.microsoft.com/cli/azure/functionapp/keys
- Hecho verificado empiricamente durante el piloto que origina este ADR, sin documentacion oficial dedicada: el registro que sirve `tools/list` en runtime (`DefaultToolRegistry`) vive en el paquete del host de la extension MCP (`Microsoft.Azure.Functions.Worker.Extensions.Mcp`), no en el ensamblado del worker -- fuente del limite estructural que la seccion 1 fija para el nivel 2. Un cambio de version de la extension que mueva ese tipo obliga a reverificar el limite antes de darlo por vigente.
- Hecho verificado empiricamente durante el mismo piloto, sin documentacion oficial dedicada: el SDK de cliente `ModelContextProtocol.Core` expone la credencial de header via `AdditionalHeaders` de las opciones de su transporte HTTP (`HttpClientTransportOptions`) -- fuente de que la Alt 2 (un `DelegatingHandler` a medida) resulto innecesaria. Verificado contra `ModelContextProtocol.Core` 2.2.0; otra version del paquete debe reverificarse antes de darse por vigente.
...
- MEF-ADR-0047 decision 1 (restriccion de coercion de argumentos string de la extension MCP, enmienda del issue #841): fuente de la restriccion que la verificacion 3 de la seccion 2 ejercita explicitamente -- el middleware de restauracion que esa decision fija no exime a la suite de nivel 3 de probar el camino valido con datos reales.
...
- 2026-08-31 (issue #789): agrega la seccion 6, doctrina de extension obligatoria de la suite ante una tool nueva o modificada -- pin del catalogo `tools/list`, pin del `inputSchema.required` y tool call real de la tool, las tres piezas obligatorias sin excepcion. Cierra el gap operacional de que ningun agente exigia esta consecuencia logica de la verificacion 2 (seccion 2): el planner no la pedia como CA, el smoke-test-writer no reconocia el proyecto de smoke MCP, y el reviewer aceptaba diferirla con el argumento invalido de "falta la key de dev".
- 2026-09-03 (issue #841): enmienda la verificacion 3 de la seccion 2 -- toda tool call real de una tool con parametros de forma fecha o identificador ejercita **cada uno** con un valor valido representativo del formato que la tool promete, y afirma que el resultado no es el mensaje de validacion; declara explicitamente que ni la tool call que omite el parametro opcional ni el error path de la verificacion 4 sustituyen ese camino. Anota en la seccion 6 que una tool nueva con parametros de esa forma hereda la misma exigencia, sin regla nueva. Cierra el gap que MEF-ADR-0047 decision 1 (misma enmienda) documenta: antes de esta enmienda, una tool call que omitia el parametro fecha y un error path con un valor que no parece fecha (`"2026-99-99"`) cumplian nominalmente las verificaciones 3 y 4 sin ejercitar nunca el camino que la coercion de la extension rompe. Evidencia empirica: consumidor pionero Bitakora.ControlAsistencia (issue #586/PR #591, mergeado), tres tools afectadas sin cobertura del camino valido hasta el smoke del issue #574.
