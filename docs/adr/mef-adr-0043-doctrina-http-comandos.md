...
Todo id que viaje como segmento de ruta debe ser **URL-safe**. En particular, el caracter `:` queda **reservado** para designar una accion de negocio (seccion 4) y nunca debe aparecer como parte de un id. Verificado contra las Microsoft Azure REST API Guidelines: *"DO restrict the characters in service-defined path segments to `0-9 A-Z a-z - . _ ~`, with `:` allowed only to designate an action operation"*, y explicitamente *"Avoid the use of the `:` character within resource ids"* [1]. El identificador de stream que ya norma MEF-ADR-0037 (Guid canonico "D" en minusculas, o clave compuesta via `ComputarStreamId`) ya cumple esta precondicion sin cambios -- ninguno de los dos formatos usa `:` ni ningun caracter fuera del conjunto permitido.
...
Esta regla de casing rige por igual **comandos y queries**: MEF-ADR-0042 y `skills/projections/read-apis.md` ya asumen minusculas en sus propios ejemplos (`programacion/turnos`), pero ningun ADR lo habia declarado como regla explicita hasta ahora -- este ADR cierra esa laguna, y es la pieza que MEF-ADR-0006 enmienda para dejar de colisionar con el ejemplo PascalCase que traia (seccion 5 abajo).
...
MEF-ADR-0011 se enmienda (control de cambios de ese ADR) para exigir, en todo issue cuyo `## Modelo de eventos` declare un comando con trigger HTTP, el **contrato HTTP** de ese comando -- verbo, ruta y el paso del test de precedencia (seccion 2 de este ADR) que se aplico -- como campo **Critico** para llegar a `estado:listo`. La seccion "Validacion en `/implement`" de ese ADR incorpora el criterio programatico correspondiente. Como esa validacion vive dentro del propio MEF-ADR-0011, la enmienda se autopropaga al gate de `/implement` sin tocar `commands/implement.md` (mismo mecanismo que ya usan las enmiendas de `tipo:projection`).
...
- **Migracion nunca forzada**: la regla de aplicabilidad (seccion 7) protege a los endpoints preexistentes de un rename automatico que rompería consumidores integrados.
...
- RFC 3986, "Uniform Resource Identifier (URI): Generic Syntax" -- IETF: el caracter `:` es legal dentro de un segmento de path/query segun la gramatica del RFC -- la proscripcion de `:` en ids de la seccion 1 es una restriccion de las guias de proveedor (Microsoft/Google), no del RFC de sintaxis de URI. https://www.rfc-editor.org/rfc/rfc3986.html
...
- MEF-ADR-0037 (identidad del stream y su representacion string canonica): el id de ruta de un comando sigue el mismo parseo tipado que ya fija ese ADR; este ADR solo agrega la precondicion URL-safe (seccion 1), sin reabrir el mecanismo de parseo.
