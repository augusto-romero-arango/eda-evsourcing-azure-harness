...
- 2026-07-06: reformado (issue #196, ancla doctrinal de la oleada de apply-en-CI, junto con ADR-0022 y ADR-0025) para fijar que el `infra-base-scaffolder` tambien emite el workflow de CI de Terraform (`infra-cd.yml`, plan en pull_request / apply en push a main) y que el `apply` real de infraestructura ocurre en ese workflow, nunca localmente. Se elimina del cuerpo la afirmacion de que `/infra aplica` (seccion "Por que un agente y no un directorio de plantillas", punto 3); el pipeline local queda acotado a escritura y revision/plan. El mecanismo concreto lo materializan la emision de `infra-cd.yml` por el scaffolder (issue #197) y la reimplementacion de `iac-pipeline.sh` y de `infra-applier` (issue #199).
