...
- Los unicos secrets de GitHub que autentican `azure/login` son **tres**: `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`. **Ninguno es un secret de password.** (Un cuarto secret, `TF_VAR_POSTGRESQL_ADMIN_PASSWORD`, alimenta una variable Terraform del `apply` -- no la autenticacion OIDC; ver "Fuente de las variables Terraform requeridas por `infra-cd.yml`" mas abajo y ADR-0025.)

### Fuente de las variables Terraform requeridas por `infra-cd.yml`

`infra/environments/<env>/variables.tf` (esqueleto emitido por `infra-base-scaffolder`, ADR-0021) declara variables sin default que `-input=false` no puede pedir de forma interactiva. El `env` del workflow las resuelve asi:
...
El desarrollador que usa Mefisto **escribe y revisa** el HCL (revision estatica: `fmt`/`init -backend=false`/`validate`); nunca ejecuta `terraform plan` ni `terraform apply` en su maquina, ni necesita permisos de Azure ni custodia secretos. El `plan` y el `apply` reales -- los unicos pasos que leen el tfstate/escriben recursos y requieren acceso a los secretos custodiados (Key Vault, tfstate) -- los ejecuta el Service Principal de CI, por OIDC, dentro de GitHub Actions (el `plan` al abrir el PR, el `apply` al mergear a `main`). **Tres perfiles de acceso, no dos (ADR-0025 decision #10):** este "cero permisos" describe el perfil (a) -- el desarrollador en su flujo *ongoing*. Hay otros dos perfiles que si manejan permisos elevados de Azure, ninguno cubierto por esta doctrina: (b) el **bootstrap** (`scripts/bootstrap-backend.sh` crea el tfstate y `scripts/setup-github-ci.sh` crea el SP y sus federated credentials), una operacion privilegiada de una sola vez que ejecuta un admin para habilitar la CI; y (c) la **siembra de secretos** de Key Vault (ADR-0025 decision #6), ahora una accion **automatica de CI** dentro del propio job `apply` (habilitada por el rol de datos M1; ver la seccion "Rol de datos de Key Vault para el propio SP de CI" mas abajo) -- se repite en cada `apply` que crea un secreto nuevo y con cada `serviceBus.external[]` que el BC declare (ADR-0024 decision #4), pero ya no es un privilegio manual de infra/admin. Ver ADR-0025 decision #10 para el detalle de los tres perfiles.
...
- **Ningun humano necesita un rol de datos de Key Vault**: el rol M1 (`Key Vault Secrets Officer` para el propio SP de CI) habilita que la siembra de secretos tambien corra en CI (ADR-0025 decision #6); elimina el footgun `ForbiddenByRbac` que un admin, aun siendo Owner, enfrentaba al intentar sembrar a mano (issue #229).
