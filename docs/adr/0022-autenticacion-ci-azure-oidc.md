...
- **[15]** Microsoft Learn, "Provide access to Key Vault keys, certificates, and secrets with Azure role-based access control" / "Quickstart: Create an Azure key vault and a secret using Bicep" — asignar un rol de datos (p. ej. `Key Vault Secrets Officer`) es requisito para operar el data plane de un vault con RBAC; los role assignments pueden tardar uno o dos minutos en propagar. https://learn.microsoft.com/azure/key-vault/general/rbac-guide
