# ADR-0026: Colas de Service Bus con sesion para fan-in y serializacion por clave de agregado
...
Lo que faltaba no era el mecanismo (documentado en la plataforma) sino la **doctrina**: cuando un agente debe decidir topic simple (fan-out, ADR-0001) vs queue con sesion (fan-in), y por que — no solo como cablear cada uno.

## Decision

### 1. Fan-out (topic) sigue siendo el default; fan-in (queue con sesion) es la excepcion deliberada

ADR-0001 no cambia: todo evento se publica a un topic por tipo de evento, y esa sigue siendo la topologia por defecto. **Topic vs queue nunca es una decision del productor** — el productor siempre publica a topics (ADR-0001, ADR-0023); el queue con sesion, cuando aplica, es una construccion exclusivamente del **lado consumidor**, downstream del topic via auto-forward.
...
Si solo se cumple la condicion 1 pero no la 2 (eventos que convergen en una decision, pero sin escritura concurrente sobre el mismo stream — por ejemplo, una agregacion de solo lectura), el topic + subscription simple de ADR-0001 sigue siendo suficiente. La excepcion de este ADR es para **serializar escritura concurrente por clave de aggregate**, no un patron general de "varios eventos, una Function".
...
El consumidor es una Azure Function con `[ServiceBusTrigger("<queue>", Connection=..., IsSessionsEnabled = true)]` sobre el queue (no sobre una subscription). El runtime de Functions procesa las sesiones en paralelo entre si, pero **serialmente dentro de cada sesion** — `maxConcurrentSessions` (default 2000) [5] acota cuantas sesiones distintas se procesan a la vez, sin romper el orden garantizado dentro de cada una. Esa serializacion por sesion es lo que elimina la escritura concurrente sobre el mismo stream de Marten: dos mensajes con el mismo `SessionId` (misma clave de aggregate) nunca se procesan al mismo tiempo, sin importar cuantos topics de origen convergieron en el queue.
...
### 4. Invariante de naming: el queue kebab-case es el nombre de la Function que lo consume, con desviacion documentada de ADR-0006

ADR-0006 nombra las Functions de Service Bus con el patron `{Accion}Cuando{Evento}` porque asume un **estimulo unico** — el nombre debe decir a que evento reacciona. Ese patron no aplica a una Function de fan-in: no hay un unico estimulo, convergen N eventos (potencialmente de N tipos) sobre la misma decision. Exigir `{Accion}Cuando{Evento}` aqui obligaria a elegir arbitrariamente uno de los N eventos convergentes, lo cual es enganoso — sugeriria que la Function solo reacciona a ese evento.

La convencion para Functions de fan-in es distinta y mas simple: **el nombre del queue en kebab-case es el nombre de la Function que lo consume**, y ese nombre describe la **decision o convergencia** que la Function resuelve (no un evento puntual). Ejemplo: un queue `consolidar-cierre-turno` lo consume una Function `ConsolidarCierreTurno`. Las subscriptions que hacen forward hacia ese queue siguen, sin cambio, el naming de ADR-0001 (`{consumidor}-escucha-{productor}`) — la desviacion aplica solo al nombre de la Function de fan-in, no a las subscriptions fuente.

### 5. Efecto sobre el manejo de errores (ADR-0004)

La serializacion por sesion **elimina una clase de error**: la carrera de concurrencia optimista de Marten entre ejecuciones paralelas del mismo handler sobre el mismo stream, que hoy termina en dead-letter ciego (sin relacion visible con el dominio). No cambia la doctrina de ADR-0004 sobre eventos de fallo vs excepciones: las reglas de negocio del aggregate se siguen modelando con eventos de fallo, nunca excepciones (ADR-0004, capa 3). Lo que este ADR resuelve es un fallo de **infraestructura de concurrencia**, no una regla de negocio — la clase de error que ADR-0004 encuadra en su capa 5 ("fallos de infraestructura"), no en la capa 3.
...
- **Complementa sin reemplazar**: ADR-0001 (fan-out por default) y ADR-0023 (namespace interno del BC) quedan intactos; el queue con sesion es una entidad de consumo mas dentro de la topologia ya existente.
...
- **Nueva excepcion a ADR-0006**: el naming de Functions de fan-in se desvia del patron `{Accion}Cuando{Evento}`, lo que exige que los agentes reconozcan explicitamente cuando aplica una convencion u otra.
- **Nueva superficie de fallo a monitorear**: `SessionLockLost` — si el procesamiento dentro de una sesion tarda mas que el lock y este no se renueva a tiempo, el receptor debe volver a aceptar la sesion [3]. No es un caso nuevo de la doctrina de ADR-0004 (sigue siendo capa 5, fallo de infraestructura), pero es una superficie a vigilar en operacion.
...
- **[5]** "Azure Service Bus bindings for Azure Functions" — Microsoft Learn. `maxConcurrentSessions` (default `2000`) en `host.json`: numero maximo de sesiones que se procesan concurrentemente por instancia escalada; el orden dentro de cada sesion se preserva. https://learn.microsoft.com/azure/azure-functions/functions-bindings-service-bus#hostjson-settings
...
- ADR-0001 (Service Bus: un topic por tipo de evento): ADR-0026 no reemplaza el fan-out por default; anade la primitiva de fan-in como excepcion deliberada, consumida exclusivamente del lado consumidor.
- ADR-0004 (manejo de errores en event sourcing): ADR-0026 elimina una clase de error de la capa 5 (fallos de infraestructura) de ADR-0004 — la carrera de concurrencia optimista de Marten — sin alterar la doctrina de eventos de fallo vs excepciones de sus capas 2-4.
- ADR-0006 (convenciones de nombramiento de Functions Azure): ADR-0026 documenta una excepcion al patron `{Accion}Cuando{Evento}` para Functions de fan-in, con la invariante "queue kebab-case = nombre de la Function".
- ADR-0023 (Bounded Context, namespace interno de ASB): el queue de fan-in vive en el namespace interno del BC, sin cambio a la doctrina de namespaces.
- ADR-0024 (modelo de eventos de bus, privado/publico): el `groupId` de `IPrivateEventSender` es el mecanismo por el que el productor fija el `SessionId` que la sesion del queue destino exige.
