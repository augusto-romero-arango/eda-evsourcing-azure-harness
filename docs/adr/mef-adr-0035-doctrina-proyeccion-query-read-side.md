...
**El `partial` es obligatorio, y es un gotcha real, poco documentado.** Verificado contra la documentación oficial de convenciones [3]: *"To make changes to an existing aggregate, declare `Apply()` methods on a `partial` projection class. The `JasperFx.Events.SourceGenerator` discovers them at compile time and emits a `[GeneratedEvolver]` dispatcher with no runtime reflection."* El propio issue de GitHub del proyecto [4] confirma empíricamente que esto aplica **también** a tipos auto-agregantes (self-hosting, no solo a una clase de proyección companion separada): un consumidor que actualizó a Marten 9 sin marcar su tipo `partial` obtuvo el error *"No source-generated dispatcher found for Marten.Events.Aggregation.SingleStreamProjection<MyType, System.Guid>"* -- el mismo síntoma que aplicaría a `TurnoView` de este ADR si se omite `partial`.

**El `partial` no alcanza solo: el ensamblado debe referenciar el analizador.** El mensaje de error completo de Marten [4] fija **dos** condiciones, no una: *"the projection class must be declared `partial` **in an assembly that references the JasperFx.Events.SourceGenerator analyzer**, or alternatively override Evolve / EvolveAsync / DetermineAction / DetermineActionAsync directly"*. Esto es directamente relevante para el marco: MEF-ADR-0034 sección 5 ubica los read models y las clases de proyección en `<RootNamespace>.ReadModels`, una **biblioteca de clases separada** del worker -- es ese `.csproj`, no el del worker ni el del write-side, el que debe arrastrar el analizador para que el dispatcher se emita. El agente que genere ese proyecto (issue #375) debe verificarlo por compilación, no por inspección visual del código: un `partial` correcto en un ensamblado sin analizador falla en runtime, no en build.

**Caveat de verificación**: el ejemplo oficial `QuestParty` que muestra la página de `aggregate-projections.html` [2] **no** declara `partial` (contra todos los ejemplos de clase de proyección companion de las páginas de single/multi-stream, que sí son uniformemente `public partial class` [3][5]) -- posible desalineamiento entre esa página ilustrativa y el comportamiento real de la versión pinneada, y el issue [4] quedó cerrado sin ningún comentario ni resolución registrada (antes de que se publicara la propia `9.12.0`), así que no zanja por sí solo el estado de la versión pinneada. El agente que implemente el primer read model real (`projection-implementer`, issue #365) debe reverificar esto empíricamente con un build antes de asumir que el ejemplo de este ADR compila tal cual -- mismo principio de re-verificación que MEF-ADR-0034 sección 6 exige para su propio config-test. **Ojo con no confundir los dos `partial` del read-side**: el de esta sección es el de la *clase de proyección/read model* (habilita el source generator de Marten); el "guarda del `partial`" de MEF-ADR-0034 sección 6 punto 1 es otro -- el *método* `partial` del seam de composición `ConfiguracionMartenProjections{Dominio}`, que puede desaparecer en silencio si nadie lo implementa. Son requisitos independientes y ambos aplican.
...
- **El requisito `partial` del estilo convencional es un gotcha poco documentado, y tiene dos mitades**: el propio ejemplo oficial de Marten (`QuestParty`) no lo muestra, y la segunda condición -- que el ensamblado de los read models referencie el analizador `JasperFx.Events.SourceGenerator` -- solo queda clara combinando la página de convenciones con el mensaje de error completo del issue de GitHub del proyecto (sección 2). Un agente que genere el primer read model real sin reverificarlo por compilación puede producir código que compila y **falla en runtime** (dispatcher ausente), no en build.
...
- **[4]** `JasperFx/marten` issue #4557 -- *"No source-generated dispatcher found for Marten.Events.Aggregation.SingleStreamProjection<MyType, System.Guid>"*: confirma empíricamente que el requisito `partial` de [3] aplica también a tipos auto-agregantes (self-hosting), no solo a una clase de proyección companion -- el repro del reportante es exactamente un snapshot que funcionaba en Marten 8 y rompió al subir a 9. El mensaje completo fija dos condiciones y la salida alterna: *"the projection class must be declared `partial` in an assembly that references the JasperFx.Events.SourceGenerator analyzer, or alternatively override Evolve / EvolveAsync / DetermineAction / DetermineActionAsync directly"*. Estado al momento de redactar este ADR: cerrado (`completed`, 2026-05-25) **sin comentarios ni resolución registrada**, y antes de la publicación de la propia `9.12.0` (2026-06-29) -- de ahí la exigencia de re-verificación por build de la sección 2. https://github.com/JasperFx/marten/issues/4557
