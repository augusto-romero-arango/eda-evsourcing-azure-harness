...
Dos restricciones que la mecánica de arriba impone y que el endpoint no puede dar por sentadas:

- **El campo del cursor debe ser comparable por el proveedor LINQ de Marten.** Si el cursor es el `Id` string de N1 (el `StreamKey`), la traducción a SQL de `CompareTo`/`>` sobre string **no está documentada** en la superficie de métodos string que Marten declara traducibles [6] -- es el cuarto gate de la sección 6. La alternativa que no depende de ese gate: ordenar por un campo naturalmente comparable de la vista (una fecha, un número) con desempate por `Id`.
- **El `Take` del cliente se acota en el servidor.** El tamaño de página llega en el body y es entrada del cliente como cualquier otra: el endpoint lo pasa por una cota propia (`Math.Clamp(filtro.Take, 1, TakeMaximo)`) antes de dárselo a Marten -- sin esa cota, un cliente pide una página sin límite y el `Take` deja de acotar nada.
...
**Regla operativa**: un agente o desarrollador que implemente el primer endpoint QUERY real de un consumidor debe tratar los cuatro puntos de la tabla como gates bloqueantes de ese primer despliegue, no como notas informativas -- mismo criterio que MEF-ADR-0032 sección 8 ya fija para sus propios puntos NO VERIFICADO de WorkOS.
...
- MEF-ADR-0032 (identidad y autenticación en el borde, WorkOS + APIM): fuente del patrón "catálogo NO VERIFICADO + gate empírico obligatorio" que la sección 6 de este ADR replica; su catálogo B1-B11 documenta además el estado actual de `<allowed-methods>` sin `QUERY` que motiva el issue de seguimiento de CORS.
