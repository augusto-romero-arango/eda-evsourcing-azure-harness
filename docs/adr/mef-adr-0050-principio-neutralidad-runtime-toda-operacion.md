...
- Un `scripts/*.sh` publicado nuevo no invoca el CLI de un runtime (`claude`, `opencode`) si puede evitarlo -- el mismo criterio que la regla R2 de `mefisto-neutrality-gate.sh` ya aplica a la fuente neutral de `src/internal/`, aunque el gate mismo no escanee `scripts/` (esta fuera de su universo de archivos, documentado en `scope_excluded` de `neutrality-allowlist.json`).
...
Agregar el runtime `<id>` (hoy `claude`/`opencode`; manana cualquier otro) es, exhaustivamente, esta lista -- ningun paso edita `src/internal/{agents,commands}/*.md`, la fuente neutral de doctrina:
...
| 3 | `src/internal/scripts/lib/adapter-<id>.sh` (traduce las directivas `{{mefisto:...}}` del contrato neutral) | `adapter-claude.sh`, `adapter-opencode.sh` |
| 4 | Rama de autodeteccion en `mefisto_resolve_runtime` (`src/internal/scripts/lib/mefisto-runtime.sh`) | ramas `has_claude`/`has_opencode` |
| 5 | Entrada en `is_path_in_mefisto_scope` (`src/internal/scripts/lib/_mefisto-common.sh`) | entradas `.claude/*`, `.opencode/*` |
| 6 | Entrada en el blocklist publicado `is_path_in_consumer_blocklist` (`scripts/_pipeline-common.sh`) | mismas rutas, lado espejo |
| 7 | Entrada(s) en `src/internal/contract/neutrality-allowlist.json` si el adaptador nuevo necesita nombrar su propio runtime en texto (mismo patron que las excepciones ya declaradas para `adapter-claude.sh`/`adapter-opencode.sh`) | seccion `exceptions` del archivo |
| 8 | Soporte `--kind <id>` en Herdr, si un pipeline orquesta ese runtime en un pane (`scripts/herdr-workspace.sh`) | `--kind claude`, `--kind opencode` |
| 9 | Tests: `test-runtime-<id>.sh` (contrato del adaptador) + cobertura de discovery equivalente a `test-opencode-discovery.sh` | `.claude/scripts/tests/test-runtime-claude.sh`, `test-runtime-opencode.sh`, `test-opencode-discovery.sh` |
...
Los **recursos de Nivel 3** (scripts, plantillas, referencias que el body de un `SKILL.md` invoca bajo demanda) se restringen al mismo toolchain que el resto del harness: bash + jq + gh (coherente con MEF-ADR-0049 CA-6), nunca Node, Python ni un binario que no este ya disponible en cualquier maquina que corra los pipelines de Mefisto.
...
**Limite conocido**: `:` es un caracter ilegal en nombres de archivo NTFS. Irrelevante mientras el harness siga siendo bash sobre macOS/Linux/WSL (MEF-ADR-0049 CA-6) -- se deja anotado para que un futuro soporte Windows nativo lo revise antes de asumir que el separador literal es portable a ese filesystem.
...
- **Bloque `[F]` de `scripts/tests/test-guards.sh`**: integridad de todo `SKILL.md` -- `name` coincide con el directorio (F1), `description` no vacia (F2), recursos de Nivel 3 referenciados existen (F3), y las referencias `skills:` de los agentes resuelven a un Skill real.
