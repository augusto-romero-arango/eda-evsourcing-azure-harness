...
- **Fecha**: 2026-09-07
- **Estado**: aceptado
- **Aplica a**: doctrina transversal a los dos lados de MEF-ADR-0019 (publicado e interno) sobre como debe reaccionar cualquier pipeline de Mefisto cuando la invocacion de un agente falla porque el proveedor de modelo agoto la ventana de uso o cayo temporalmente. La implementan el lado interno (`src/internal/scripts/mefisto-tooling-pipeline.sh`, `src/internal/scripts/lib/_mefisto-common.sh`) y, desde el issue #971, el lado publicado (`scripts/_pipeline-common.sh` -- `classify_agent_failure`/`agent_failure_is_holdable`/`agent_hold_wait` -- consumido por `tdd-pipeline.sh`, `tooling-pipeline.sh`, `iac-pipeline.sh` y `scaffold-pipeline.sh`). La doctrina no es propiedad de ninguno de esos archivos: es el criterio con el que se alinea todo pipeline que invoque agentes, en cualquiera de los dos lados. Cross-referencia MEF-ADR-0049 (arquitectura neutral runtime/proveedor: la deteccion que esta decision presupone vive detras del adaptador que ese ADR define), MEF-ADR-0050 (principio de neutralidad: la politica se decide sobre el vocabulario cerrado `error.kind` del contrato neutral, nunca sobre el texto de un runtime concreto) y MEF-ADR-0019 (separacion publicado/interno: esta doctrina rige en ambos lados aunque hoy un solo archivo la ejecute).
...
- **La reanudacion de sesion del lado publicado (decision 5).** Ningun pipeline publicado la implementa todavia: tras una espera, la sonda repite el stage desde cero. Este ADR fija el criterio que esa portacion debe seguir; no la ejecuta.
...
- **El lado publicado implementa la doctrina con una deteccion mas debil que la del interno.** Sin el JSONL neutral, `classify_agent_failure` publicada (issue #971) depende de texto: el evento estructurado `rate_limit_event` solo esta disponible cuando la corrida capturo el stream (`PIPELINE_CAPTURE_STREAM=true`), y con `--output-format text` un limite de uso solo se reconoce si el log trae a la vez el codigo 429 y un indicio de "rate limit"/"usage limit" -- el mismo patron conservador del fallback de `runtime-opencode.jq`, que prefiere no clasificar antes que producir un falso positivo. Un limite de uso que no deje esa huella cae en `CLI_ERROR` y aborta el stage sin esperar, exactamente como antes de #971. Tampoco hay ahi reintento corto (#534) ni reanudacion de sesion (#968): la sonda repite el stage desde cero.
