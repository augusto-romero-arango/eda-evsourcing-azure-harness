...
`classify_agent_failure` (`_mefisto-common.sh:1332-1369`) traduce el desenlace de una invocacion fallida a una de seis etiquetas. Cada una tiene exactamente un trato, decidido por tres funciones independientes que el bucle de `run_agent` (`mefisto-tooling-pipeline.sh:681-904`) consulta en orden -- reintento corto primero, espera despues, abortar como default:
...
|---|---|---|---|
| `TIMEOUT` | no | no | **si** (watchdog agoto `timed_out=true`) |
...
El hold (`mefisto-tooling-pipeline.sh:609-904`) fija tres parametros, todos overridables por variable de entorno para que los tests lo ejerzan sin esperar horas reales:

- **Sonda (`MEFISTO_HOLD_PROBE_SECONDS`, default 300s = 5 min)**: el piso garantizado de cadencia de sondeo cuando el terminal del intento fallido no informa `resets_at`. Cuando si lo informa (Claude Code lo hace para `RATE_LIMIT`), el hold duerme hasta ese instante exacto mas un margen fijo de 60s (`HOLD_RESET_MARGIN_SECONDS`) en vez de sondear a ciegas -- despertar justo en el segundo cero puede ganarle por poco a una ventana todavia cerrada.
...
**La espera no consume el watchdog del stage ni el presupuesto de reintentos**, por diseño y no por accidente: `HOLD_TOTAL_SECONDS`/`HOLD_ELAPSED_SECONDS` son contadores propios, independientes de `attempt`/`MAX_ATTEMPTS` (`_mefisto-common.sh:1426-1436`, `mefisto-tooling-pipeline.sh:633-643`) -- son dos politicas sobre dos causas distintas, y una no debe pagar el precio de la otra. Ademas, el `sleep` del hold ocurre en el bucle del pipeline, **fuera** de la invocacion a `run_agent_with_watchdog` de cada intento: cada intento nuevo arranca con su propio watchdog de `AGENT_TIMEOUT_SECONDS` completo, sin arrastrar el tiempo ya gastado esperando.
...
Hoy los dos runtimes soportados lo implementan: Claude Code via `--resume <session-id>`/`-r` (`runtime-claude.sh:76-82`) y OpenCode via `-s`/`--session <id>` (`runtime-opencode.sh:90-101`, verificado en `opencode run --help` local). Un runtime nuevo que se agregue siguiendo el checklist de MEF-ADR-0050 seccion 2 sin implementar esta funcion simplemente cae siempre en el caso (b) -- pierde la reanudacion, no la espera: el hold sigue funcionando (duerme y reintenta desde cero), solo sin la continuidad de sesion.
...
- Issue #967: la politica de espera (hold) -- `agent_failure_is_holdable`, los defaults de sonda/techo, y el bucle de espera del pipeline. `_mefisto-common.sh:1411-1447`, `mefisto-tooling-pipeline.sh:609-643` y `:791-904`.
- Issue #968: la reanudacion de sesion tras un hold -- captura de `session_id`, los tres casos de degradacion, y el prompt corto de continuacion. `mefisto-tooling-pipeline.sh:663-680` y `:848-892`; contrato en `src/internal/contract/README.md:541-566`.
...
- `src/internal/scripts/lib/runtime-claude.sh:76-82` y `src/internal/scripts/lib/runtime-opencode.sh:90-101`: soporte de reanudacion verificado por runtime (`--resume`/`-r` en Claude Code, `-s`/`--session` en OpenCode).
