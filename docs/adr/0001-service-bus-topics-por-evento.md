...
Un eje distinto -- no cuantos tipos de evento sino cuantos **destinatarios** de un mismo evento -- tiene tambien una excepcion gobernada al rechazo de filtros SQL de este ADR: cuando un unico evento publico debe llegar a N destinatarios que solo quieren su subconjunto, cada subscription destinataria puede llevar un **correlation filter de igualdad** sobre una clave de enrutamiento (application property), sin recurrir a un filtro SQL — ver ADR-0027. El escape-hatch `filter_type = "SqlFilter"` que el modulo `service-bus` de `infra-base-scaffolder` expone hoy para el campo `sub.filter` no es el mecanismo que ADR-0027 fija para este eje; ese modulo se reconcilia en el trabajo de infraestructura diferido de ADR-0027. El rechazo de filtros SQL de este ADR no se reabre: sigue aplicando, sin excepcion, a cualquier caso que no sea igualdad exacta.
...
- Sin filtros SQL: el rechazo se mantiene sin excepcion (ADR-0027 acota este absoluto con un
...
- 2026-07-15: enmendado (issue #275, doctrina fundacional de ADR-0027) para acotar, sin eliminarlo, el absoluto "sin filtros SQL / suscribirse a un topic es querer todo": se anade en "Decision" el parrafo que reconoce el eje "un evento, N destinatarios" y la excepcion gobernada de un correlation filter de igualdad (ADR-0027), reconciliando el escape-hatch `filter_type = "SqlFilter"` latente del modulo `service-bus` de `infra-base-scaffolder`, cuyo ajuste queda diferido al trabajo de infraestructura de ADR-0027; se ajusta la consecuencia "Sin filtros SQL" en la misma direccion y se agrega la referencia cruzada a ADR-0027.
