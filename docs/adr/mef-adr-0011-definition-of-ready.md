...
**Nota sobre `projection` y el routing del pipeline**: esta validacion programatica verifica completitud de informacion, **no** disponibilidad del pipeline read-side. El enrutamiento por label todavia no conoce el tipo: hoy `tipo:projection` cae en el caso por defecto del resolver (`feature`/`refactor` -> pipeline TDD, `tooling` -> pipeline de tooling, `infra` -> SKIP), asi que `/implement` no lanza los agentes read-side (`projection-test-writer`/`projection-implementer`) **ni** los write-side: **aborta** antes de crear la sesion tmux con `Issue #N no se puede enrutar a un pipeline (no-tipo)`. Un issue `projection` puede entonces pasar el DoR y quedar `estado:listo` legitimamente, pero su implementacion se coordina aparte hasta que se cierren los issues #371 (rama read-side de `tdd-pipeline.sh`) y #372 (enrutamiento de `tipo:projection` en los routers). El `planner` debe advertirselo al usuario antes de marcar `estado:listo` (`agents/planner.md`, seccion "Necesidades de lectura y proyecciones").
