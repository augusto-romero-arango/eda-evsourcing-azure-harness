...
6. Si el issue introduce o modifica un endpoint HTTP de comando (en `feature`, declarado dentro de `## Modelo de eventos`; en `refactor`, dentro de `## Contexto` o `## Impacto en archivos`), el body declara para ese comando el verbo HTTP, la ruta y el paso del test de precedencia de MEF-ADR-0043 aplicado -- dentro de la propia descripcion del comando, sin encabezado nuevo
