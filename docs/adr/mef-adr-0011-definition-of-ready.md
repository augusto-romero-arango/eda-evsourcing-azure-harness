...
| Contrato HTTP del comando | Condicional | Condicional | No aplica | No aplica | No aplica |
...
**Nota sobre el Contrato HTTP de comandos (MEF-ADR-0043)**: la fila "Contrato HTTP del comando" es *Condicional* -- pasa a **Critico** cuando el issue **introduce o modifica un endpoint HTTP de comando**; es *No aplica* cuando no toca ninguno (una reaccion pura a un evento de ServiceBus, por ejemplo). Es la unica fila de la tabla que **no** nombra un encabezado del body, y a proposito: no exige una seccion nueva, sino que la informacion viva donde el issue ya describe ese comando -- en `feature`, dentro de `## Modelo de eventos` (de ahi que la condicion se lea casi siempre contra esa seccion); en `refactor`, que no declara modelo de eventos, dentro de `## Contexto` o `## Impacto en archivos`. Lo exigido es, para cada comando HTTP afectado: el verbo (`POST`/`PUT`/`DELETE`), la ruta REST completa (kebab-case minusculo, MEF-ADR-0043 seccion 3), el paso del test de precedencia de MEF-ADR-0043 seccion 2 que justifica esa eleccion, y el **codigo de exito** de la respuesta sincrona (`201`/`204`/`200`, o `202 Accepted` cuando el procesamiento solicitado continua despues de responder, MEF-ADR-0004). El codigo no se da por deducido del paso: el paso acota el conjunto admisible y fija el default, pero el issue declara cual de ese conjunto eligio (el paso 2 admite tanto el `204` canonico como el `201` de una creacion real por PUT; el paso 4 se bifurca en `204`/`200`/`202` segun si la accion devuelve representacion y si el procesamiento continua despues de responder). Cuando el codigo declarado es `202 Accepted`, el contrato agrega ademas la **razon concreta** por la que el procesamiento solicitado no termino al responder -- que trabajo queda pendiente y por que no completo antes de responder -- sin esa razon, `202` no es una opcion valida del contrato. En `refactor` el caso tipico no es un endpoint nuevo sino el rename de uno viejo hacia la forma conforme, que MEF-ADR-0043 seccion 7 admite solo discutido con el humano y nunca forzado -- el contrato declarado en el issue **es** el registro de esa discusion.
...
- **Contrato HTTP de comandos** (MEF-ADR-0043): input directo del `test-writer`/`implementer` para fijar el verbo HTTP y la ruta REST del endpoint de comando -- sin el, el agente adivina entre POST/PUT/DELETE y el casing de la ruta, exactamente el drift que motivo MEF-ADR-0043 (casing mixto entre dominios, dos estilos de comando conviviendo sin criterio escrito). El reviewer usa el paso del test de precedencia declarado para verificar que el verbo elegido corresponde al comando (p. ej. que un comando que en realidad reemplaza un value object atomico no quedo expuesto como `POST` en vez de `PUT`). El **codigo de exito** es el cuarto elemento del contrato y es input directo de cuatro agentes distintos, sin que ninguno asuma un valor por defecto: el `test-writer` lo usa para afirmar el status code de la respuesta sincrona en el test de precedencia; el `implementer` lo usa para fijar el `[HttpResult]`/status literal del endpoint; el `smoke-test-writer` lo usa para afirmar el status code observado end-to-end contra el entorno desplegado; el `reviewer` lo usa para verificar que el codigo implementado corresponde al paso de precedencia declarado (p. ej. que una accion que no devuelve representacion no quedo respondiendo `200` en vez de `204`) y que un `202 Accepted` trae su razon concreta -- sin ella, el reviewer rechaza el `202` como opcion no valida del contrato (MEF-ADR-0043 seccion 6, MEF-ADR-0004). Sin el codigo declarado, los cuatro agentes vuelven a adivinar entre `200`, `201`, `202` y `204` -- exactamente el riesgo que este campo cierra.
...
6. Si el issue introduce o modifica un endpoint HTTP de comando (en `feature`, declarado dentro de `## Modelo de eventos`; en `refactor`, dentro de `## Contexto` o `## Impacto en archivos`), el body declara para ese comando el verbo HTTP, la ruta, el paso del test de precedencia de MEF-ADR-0043 aplicado y el codigo de exito de la respuesta sincrona -- y si ese codigo es `202 Accepted`, tambien la razon concreta de por que el procesamiento no termino al responder -- todo dentro de la propia descripcion del comando, sin encabezado nuevo
...
- 2026-08-14: enmendado (issue #621, creacion de MEF-ADR-0043). Suma la fila "Contrato HTTP del comando" a la tabla DoR (*Condicional* en `feature`/`refactor`, *No aplica* en `infra`/`tooling`/`projection`) con su nota: pasa a **Critico** cuando el issue introduce o modifica un endpoint HTTP de comando, exigiendo verbo + ruta + el paso del test de precedencia de MEF-ADR-0043 que se aplico. Es la unica fila que no nombra un encabezado del body, a proposito: la informacion vive donde el issue ya describe el comando (`## Modelo de eventos` en `feature`; `## Contexto`/`## Impacto en archivos` en `refactor`, que no declara modelo de eventos) -- por eso la condicion no se ata a la presencia de una seccion que la propia tabla marca *No aplica* en `refactor`. Suma el bullet correspondiente en "Por que cada campo critico" y el criterio 6 de "Validacion en `/implement`". Esa validacion vive dentro de este mismo ADR y `commands/implement.md` la delega leyendola en runtime, asi que la enmienda se autopropaga al gate sin duplicar doctrina en el skill; el unico ajuste que si necesito el skill fue de conteo (enumeraba "los 5 criterios", cantidad fija memorizada fuera del ADR que habria dejado el criterio 6 fuera del gate en silencio, ahora "todos los criterios que enumere esa seccion").
- 2026-09-07: enmendado (issue #991, depende de #990) para exigir el **codigo de exito** como cuarto elemento del contrato HTTP, cerrando el hueco que #849/#990 abrieron en MEF-ADR-0043 (el test de precedencia ya fija el codigo junto con verbo y ruta) sin que este ADR lo reflejara: hasta esta enmienda, un issue podia llegar a `estado:listo` declarando verbo + ruta + paso sin declarar el codigo, y `test-writer`/`implementer`/`smoke-test-writer`/`reviewer` volvian a adivinar entre `200`, `201`, `202` y `204`. La nota sobre el Contrato HTTP de comandos ahora exige, ademas de verbo/ruta/paso, el codigo de exito de la respuesta sincrona -- sin darlo por deducido del paso, porque el paso 2 admite `204` o `201` y el paso 4 se bifurca en `204`/`200`/`202` -- y, cuando el codigo declarado es `202 Accepted`, la razon concreta de que procesamiento queda pendiente y por que no completo antes de responder (sin ella, `202` no es una opcion valida del contrato, MEF-ADR-0004). "Por que cada campo critico" nombra explicitamente a los cuatro agentes consumidores del codigo de exito, sin asumir default. El criterio 6 de "Validacion en `/implement`" suma la presencia del codigo y, para `202`, de su justificacion, dentro de la misma descripcion del comando -- sin encabezado nuevo y sin tocar `commands/implement.md`: ese skill ya delega dinamicamente ("todos los criterios que enumere esa seccion", ajuste del issue #621) desde que se corrigio el conteo fijo que habria dejado el criterio 6 fuera del gate en silencio, asi que esta enmienda se autopropaga al gate sin duplicar doctrina en el comando.
