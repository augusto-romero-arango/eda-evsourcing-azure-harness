...
if [ "$1" = pr ] && [ "$2" = view ]; then cat "$MERGE_SHA_FILE"; exit 0; fi
exit 1
EOF
    ...
    (
        ...
        env -u MEFISTO_STATE_DIR -u MEFISTO_LEGACY_STATE_DIR -u MEFISTO_REPO_ROOT \
            ...
            MERGE_SHA_FILE="$name/merge-sha" MEFISTO_RUNTIME=claude PATH="$bin:/usr/bin:/bin:/usr/sbin:/sbin" \
...
if [ "$LAST_RC" -eq 0 ] && grep -qF 'reconciliacion post-merge de bloqueados fallo' "$LAST_DIR/out"; then
...
if grep -qx -- '--reconcile-pr 202' "$LAST_DIR/reconcile.log" \
    && [ "$(grep -c '^pr merge ' "$LAST_DIR/gh.log")" -eq 2 ] \
    && [ "$(grep -c '^pr view ' "$LAST_DIR/gh.log")" -eq 2 ]; then
    pass "el siguiente eslabon y su sync continuan sin reintento"
else
