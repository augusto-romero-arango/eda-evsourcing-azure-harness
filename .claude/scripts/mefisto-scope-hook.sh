...
{
    echo "FUERA DE SCOPE: '$REL_PATH' no esta en la allowlist interna de Mefisto (is_path_in_mefisto_scope, src/internal/scripts/lib/_mefisto-common.sh). El gate final del stage (validate_mefisto_scope_changes) abortara el PR si el archivo sigue ahi."
    ...
    echo "Rutas permitidas: commands/, skills/, agents/, scripts/, hooks/, docs/, .claude-plugin/, .claude/{commands,skills,agents,scripts}/, .claude/settings.json, .mcp.json, src/{internal,published,runtime}/, dist/, .opencode/{agents,commands,plugins,skills}/, AGENTS.md, opencode.json, changelog.d/, README.md, CHANGELOG.md, CLAUDE.md, .gitignore."
