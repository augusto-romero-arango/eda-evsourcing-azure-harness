...
5. Actualiza el CHANGELOG: como parte de implementar el issue, anade una entrada bajo '## [Unreleased]' en CHANGELOG.md siguiendo Keep a Changelog, con la categoria correcta ('Added' para funcionalidad nueva, 'Changed' para cambios de comportamiento, 'Fixed' para bugs, 'Removed' para eliminaciones). CRITICO: '## [Unreleased]' es SIEMPRE la PRIMERA seccion del archivo (antes de cualquier '## [x.y.z]'). Coloca la entrada ahi, NUNCA bajo una seccion de version publicada como '## [0.8.0]' o similar -- aunque esa seccion tenga una subseccion '### Changed' o '### Added' justo debajo y parezca tentador insertarla ahi. Si falta la subseccion de categoria ('### Added', '### Changed', '### Fixed', '### Removed') dentro de '## [Unreleased]', crealá ahi. Excepcion: si el cambio toca exclusivamente bitacora (docs/bitacora/**) u otros archivos de gobierno no notables (README.md, CLAUDE.md, .gitignore), omite la entrada. Un gate del pipeline aborta el PR si un cambio notable llega sin entrada en [Unreleased] o si la entrada quedo bajo una seccion de version publicada.
