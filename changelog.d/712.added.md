- El pipeline `tdd-pipeline.sh` (lado publicado) acepta el flag
  ...
  honran el mismo mapa por clave) sigue sin agregar `--model`: comportamiento
  byte a byte el actual. Con override, se verifico empiricamente (evento
  `init` del stream-json de una corrida real con `--agent test-writer --model
  haiku`) que `--model` del CLI precede al frontmatter del agente. El scaffold
  ...
  que gobierna solo los stages con agente.
