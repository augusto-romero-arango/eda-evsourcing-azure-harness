- El pipeline `tooling-pipeline.sh` (lado publicado) acepta el flag
  ...
  pipeline no hace push, no abre PR y no comenta el issue -- la rama queda
  local, y el resumen final explica como promoverla a mano si esa variante
  gana la comparacion. Se combina con `--models` (una variante sin `--models`
