...
1. **Crear el backend del tfstate** con `bootstrap-backend.sh` (idempotente; crea Resource Group `rg-<proyecto>-tfstate`, Storage Account endurecida —con un **sufijo de unicidad global** sobre el nombre base de `terraformStateStorage`, ver la nota de ese campo arriba— y container `tfstate`, y escribe `infra/environments/<env>/backend.tf` con el nombre final resuelto):
   ...
   > **Primera corrida: rol de datos requerido.** La primera vez que corres `bootstrap-backend.sh`, el paso que crea el container `tfstate` usa `--auth-mode login` (acceso por AAD, sin access key): tu identidad de Azure (`az login`) necesita el rol de datos **`Storage Blob Data Contributor`** sobre la Storage Account del tfstate, además de los permisos de control plane para crearla. Si eres `Owner` de la suscripción/resource group ya lo tienes implícito; si solo eres `Contributor` (que no otorga acceso a datos de blobs), asígnatelo tú mismo o pide que te lo asignen antes de correr el script. El script lo advierte si falla (`scripts/bootstrap-backend.sh:284`) y es idempotente: reintenta tras la propagación RBAC (unos minutos).
...
2. **Configurar la autenticación de CI hacia Azure** con `setup-github-ci.sh`. Usa **OIDC** (Workload Identity Federation, ver **ADR-0022**): crea el Service Principal de GitHub Actions **sin secret** y le asigna, a nivel suscripción, `Contributor` (deploy de Functions e infraestructura) y `Role Based Access Control Administrator` con una condición ABAC anti-escalación (para que el `apply` de CI pueda crear los role assignments que emiten los scaffolders — **ADR-0025** — sin poder auto-asignarse `Owner`, `User Access Administrator` ni el propio rol); y sobre el tfstate ya creado, `Storage Blob Data Contributor` (lectura+escritura por AAD, backend keyless) — resuelve el nombre **final** de la Storage Account —con el sufijo que le puso el bootstrap— leyéndolo del `backend.tf` recién escrito, así que la asignación de rol apunta a la cuenta real, no al nombre base del config. Le añade además **dos federated credentials**: uno que confía en la rama `main` del repo (deploy de código y `apply` de infra) y otro en el evento `pull_request` (el `terraform plan` que corre en cada PR). Por eso corre **después** del paso 1:
   ...
   > **Idempotencia parcial de la condición ABAC.** `az role assignment create` es idempotente para asignaciones sin condición (la de `Contributor` de arriba), pero **no** para la de `Role Based Access Control Administrator` con la condición anti-escalación: si tu repo ya tenía esa asignación creada por una versión de este script **anterior al issue #195** (sin `--condition`), re-correr `setup-github-ci.sh` detecta la asignación existente (mismo rol/alcance/principal) y la deja tal cual — **no** le aplica la condición retroactivamente. El SP quedaría con el rol sin la restricción anti-escalación. **Diagnóstico**: `az role assignment list --assignee <appId> --query "[?roleDefinitionName=='Role Based Access Control Administrator'].condition"` — si sale vacío, borra la asignación (`az role assignment delete`) y vuelve a correr el script para que la recree **con** la condición. El arreglo del script (detectar y recrear automáticamente) queda diferido a un issue aparte.
