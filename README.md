...
- **Skills** (slash commands): `/onboard`, `/upgrade`, `/implement`, `/tooling`, `/infra`, `/infra-base`, `/scaffold`, `/scaffold-projections`, `/scaffold-mcp`, `/seed-secret`, `/install-workos`, `/install-apim`, `/install-auth`, `/parallel`, `/next-order`, `/sequential`, `/bug`, `/draft`, `/fix-review`, `/health-check`, `/work-status`, `/eraser-diagram`, `/merge`, `/bitacora`, `/purge-store`.
