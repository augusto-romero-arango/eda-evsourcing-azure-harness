...
Dentro del lado interno, `src/internal/{agents,commands}/*.md` es la **fuente neutral** (MEF-ADR-0049): `.claude/agents/*.md` y `.opencode/agents/*.md` son su **salida generada** por `src/internal/scripts/generate-internal-adapters.sh` y **no se editan a mano** — se edita la fuente y se regenera (`--check` falla si divergen). Los comandos (`.claude/commands/`), los Agent Skills internos (`.claude/skills/`) y los pipelines (`.claude/scripts/`) todavia se escriben a mano; su migracion sigue pendiente.
