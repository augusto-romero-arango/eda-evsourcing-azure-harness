...
4. **¿Filtros, ordenamientos, cadencia y paginación?** Qué se busca, cómo se ordena, con qué frecuencia se consulta, volumen esperado y modo de navegación (páginas vs cursor), combinaciones de filtros. Con la respuesta, aplica el checklist de la subsección siguiente para llegar a un verbo, un shape de filtro y una estrategia de paginación concretos -- la mecánica con la que se materializan (códigos HTTP, forma del `HttpTrigger`, APIs de Marten) es alcance del implementer, no de esta receta.

### Sugerir verbo, filtro y paginación (MEF-ADR-0042)

No te limites a capturar la respuesta a la pregunta 4: **propón el verbo, el shape del filtro y la estrategia de paginación** -- mismo estilo generativo que las cuatro preguntas anteriores (MEF-ADR-0008). MEF-ADR-0042 es la fuente de verdad de la mecánica completa (códigos HTTP 400/415/422, forma del `HttpTrigger`, APIs de Marten); esta receta replica solo el checklist decidible para aplicarlo en la conversación, sin re-preguntar nada que el implementer ya resuelve con la doctrina del ADR + Agent Skill `projections`.

**Checklist GET vs QUERY** (MEF-ADR-0042 sección 1 -- una única pregunta, sin zona gris): ¿el filtro que acabás de capturar es un conjunto de pares `campo=valor` en igualdad, sin rango ni combinación lógica? Si sí, sugiere **GET**; si no -- aparece un rango, una lista de valores, un OR, o un cursor -- sugiere **QUERY**. Propón el verbo con el propio filtro capturado como evidencia, nunca sin verbo sugerido:
...
Con las respuestas, propón el shape del DTO de filtro en el vocabulario del actor de la vista (el mismo acuñado en la pregunta 1) -- nunca en el de las columnas de la tabla o el evento de origen: "para un supervisor que filtra turnos, ¿el filtro sería sede + rango de fechas + lista de estados?". El DTO en sí (tipo C#, códigos 400/415/422) es mecánica del ADR -- no la escribas aquí, solo el shape en prosa.

**Paginación**: pregunta el volumen esperado (¿decenas, miles, sin cota?) y el modo de navegación que el consumidor de la pregunta 1 necesita -- ¿cursor "siguiente" o saltos por número de página? -- y qué campo da un orden estable (fecha, id monótono). Propón **keyset/cursor por default** (MEF-ADR-0042 sección 2, coherente con una colección event-sourced que crece sin cota) y presenta **offset como excepción**: solo si el experto documenta una necesidad real -- p. ej. un panel administrativo con navegación "página N de M" sobre una colección acotada -- bajo la misma carga de la prueba que cualquier otra excepción del marco (Rule of Three, MEF-ADR-0018).
...
- **Endpoints/rutas**: qué Functions de query expone (`Obtener{TerminoVista}` por id y/o `Listar{TerminoVista}s` por filtro), con su ruta REST -- naming fijado por MEF-ADR-0006 y el recurso `naming.md` del Skill. Por cada endpoint con filtro, registra el resultado de "Sugerir verbo, filtro y paginación" arriba: **verbo sugerido** (GET o QUERY, MEF-ADR-0042 sección 1), **shape del filtro** (campos y su combinación AND/OR, rangos, listas) y **estrategia de paginación** (keyset con su campo de orden, u offset con la justificación del experto) -- la mecánica de implementación (códigos HTTP, `HttpTrigger`, APIs de Marten) es alcance del implementer, no de este registro. Ejemplo:

  > `Listar{TerminoVista}s` -- **QUERY** (rango de fechas + lista de sedes). Filtro: `Sede` (igualdad), `DesdeFecha`/`HastaFecha` (rango cerrado), `Estados` (lista, OR). Paginación: keyset por `Id` (StreamKey), cursor "siguiente".
...
- **Filtro**: [campos y su combinacion AND/OR, rangos abiertos/cerrados, listas de valores -- resultado de "Sugerir verbo, filtro y paginacion"]
- **Paginacion**: [keyset por `{campo}` con cursor "siguiente" (default, MEF-ADR-0042 seccion 2) | offset justificado: {razon documentada por el experto, Rule of Three MEF-ADR-0018}]
