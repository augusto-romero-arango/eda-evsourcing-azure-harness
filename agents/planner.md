...
3. **¿Remueve verazmente un sub-recurso direccionable, sin payload?** Dos condiciones, ambas necesarias: el sub-recurso deja de ser legible en el estado vigente, y el comando no necesita datos además del id de ruta. → `DELETE {recurso}/{sub-recurso}`, **`204 No Content`** (también en el no-op por estado ya alcanzado, MEF-ADR-0004).
...
- **Comando**: `NombreComando` (trigger: HTTP | ServiceBus)
  ...
  - **Codigo de exito** (solo si trigger: HTTP -- MEF-ADR-0004/MEF-ADR-0043): `[201|204|200|202]` -- ej. `204 No Content (sin representacion)`. Si es `202 Accepted`, agrega la **justificacion verificable** de que procesamiento queda pendiente y por que no completa antes de responder; sin ella, `202` no es una opcion valida del contrato.
