...
**Parámetros de forma fecha o identificador: siempre `string`, nunca `DateTimeOffset`/`object`.** Si alguna tool del catálogo recibe una fecha o un identificador (GUID, código), su parámetro se declara `string` y se valida explícitamente en el cuerpo de la tool (`TryParseExact`/`Guid.TryParse`) -- nunca `DateTimeOffset` ni `object`. La extensión `Microsoft.Azure.Functions.Worker.Extensions.Mcp` coerciona todo argumento entrante a fecha/GUID sea cual sea el tipo declarado del parámetro, y el middleware que `/scaffold-mcp` genera y cablea siempre (`ArgumentosCrudosMcpMiddleware`, #840) solo puede restaurar el texto original cuando el parámetro es `string` (restricción de coerción, MEF-ADR-0047). Un issue que proponga `DateTimeOffset`/`object` para uno de estos parámetros es un error de protocolo -- re-derívalo antes de pasar al handoff.
