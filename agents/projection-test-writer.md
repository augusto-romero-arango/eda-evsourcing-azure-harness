...
using JasperFx.Events;   // Event<T> vive en JasperFx.Events, NO en Marten.Events -- mismo gotcha de
                          // namespace que MEF-ADR-0034 seccion 6 ya documenta para DaemonMode (CS0103
                          // si se importa Marten.Events por costumbre).
...
`previa.Id` ya no es el argumento del evento de cierre: `TurnoView.Id` es `string` (el stream key), pero `TurnoCerrado` sigue llevando su propio campo de dominio en `Guid` (mismo patron que `MarcacionRegistrada(Guid EmpleadoId, ...)` de `test-writer.md`) -- son dos identidades de naturaleza distinta, no una coincidencia de tipo a reusar entre ambas.
