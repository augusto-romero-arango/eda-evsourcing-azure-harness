...
- **Codigo de exito coincide con el contrato del issue** (MEF-ADR-0011 cuarto elemento; MEF-ADR-0013 seccion "Codigo de exito esperado"): para cada smoke test del camino feliz, el status code que asierta es exactamente el que el contrato HTTP del issue declaro para ese endpoint -- nunca un valor generico ni el `202` "de siempre". Tratar `202`, `201`, etc. como ejemplos intercambiables de "operacion exitosa", en vez de exigir el codigo especifico que el issue pacto, es **defecto bloqueante**. Ademas, para `201` verifica (si el contrato exige `Location`) que el test lea ese header contra la URI canonica de lectura sin dereferenciarla (una proyeccion `Async` puede materializarla despues, MEF-ADR-0004/MEF-ADR-0034); para `204`, que verifique el body vacio; para `200`, que verifique la representacion pactada, no reglas de negocio (esas ya las cubren los unit tests). **Excepcion**: los clientes HTTP tipados que un servidor MCP usa hacia las Function Apps del BC (`IsSuccessStatusCode`/`EnsureSuccessStatusCode`) no fijan un codigo puntual y toleran correctamente cualquier `2xx` contractual -- no son objeto de este punto.
