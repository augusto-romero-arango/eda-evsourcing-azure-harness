...
3. **Ids URL-safe: `:` reservado para acciones** (MEF-ADR-0043 seccion 1). Ningun id que viaje como segmento de ruta contiene `:` -- el caracter esta reservado exclusivamente para separar el verbo de accion (`{codigo}:terminar`). Un id compuesto que use `:` como separador interno en vez del formato que ya exige `ComputarStreamId` (MEF-ADR-0037) es hallazgo bloqueante.
...
5. **`Route` explicito, nunca el default del atributo** (MEF-ADR-0006, seccion "Ruta HTTP"). Un `HttpTriggerAttribute` sin `Route` resuelve a `/api/{NombreDeLaFunction}`, nunca al recurso REST que exige el ADR. Ausencia de `Route` explicito es hallazgo bloqueante.

6. **Verbo declarado explicitamente en ambos lados cuando dos Functions comparten segmento de ruta** (MEF-ADR-0006, seccion "Cada Function declara su verbo, siempre"). El `Methods` del `HttpTriggerAttribute` es opcional y, si se omite, la Function responde a todos los metodos -- una Function que omite su verbo captura tambien las demas que comparten el mismo segmento (ej. `PUT`/`DELETE` sobre el mismo `.../etiquetas/{categoria}`, o dos acciones `POST {recurso}:{verbo}` sobre el mismo recurso). Si alguna de las dos Functions que comparten segmento omite el metodo o lo deja implicito, es hallazgo bloqueante. Esto verifica solo la declaracion explicita -- la convivencia real de dos Functions con plantilla identica y verbos disjuntos en el host de Azure Functions sigue como gate empirico segun el propio MEF-ADR-0006 (mismo parrafo), no es objeto de este item.

7. **Coincide con el contrato HTTP pactado en el issue** (MEF-ADR-0011 enmendado por MEF-ADR-0043 seccion 6). El campo "Contrato HTTP del comando" del issue (verbo + ruta + paso de precedencia aplicado) es el contrato contra el que contrastas el codigo, no una sugerencia tuya. Si el codigo diverge de ese contrato -- otro verbo, otra ruta -- sin que el implementer haya documentado la razon en su resumen de fase verde, es hallazgo bloqueante. **Fallback -- issue legado sin el campo** (trabajo iniciado antes de MEF-ADR-0043): verifica igual los puntos 1-6; omite este punto 7 (no hay contrato pactado contra el cual contrastar) y anotalo en el reporte.

**Endpoints preexistentes que el diff toca pero no crea (MEF-ADR-0043 seccion 7)**: **nunca** hallazgo bloqueante, sin importar cuantos de los puntos 1-6 incumplan -- pueden tener consumidores externos ya integrados contra la URL vieja, y renombrarla es un breaking change. A lo sumo, una sugerencia no bloqueante que (a) advierte explicitamente el riesgo de breaking change y (b) remite la decision de migrar al humano via el planner. Nunca la apliques de oficio ni la reportes como hallazgo que bloquee un PR no relacionado con ese endpoint.
