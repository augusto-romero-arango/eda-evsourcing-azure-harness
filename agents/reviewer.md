...
|---|---|---|---|
| ej: `IntervaloTemporalSerializacionTests.RoundTrip_*` | Contradiccion estructural: `Contracts.Tests` no puede usar `CrearOpcionesMarten()` (`ControlHoras.Infraestructura`) | Archivo eliminado: el refactor del issue volvio imposible la precondicion del test sin violar MEF-ADR-0012 | CA-5 cubierto por `IntervaloTemporalSerializacionMartenTests` en `ControlHoras.Tests/Infraestructura/` |
```
...
4. **`InternalsVisibleTo` de Contracts hacia un proyecto de dominio**. Es proscrito por MEF-ADR-0012 (regla #4 implementer.md). La logica de conversion vive en el VO via metodo publico (`ToDetalle()`, `ToDto()`).
...
**Antes de flagear una ausencia, lee la frontera de propagacion** (MEF-ADR-0038, "Aplica a"): ese ADR fija la doctrina y delega su propagacion al codigo generado en issues propios, que no aterrizan a la vez. Hoy la tiene propagada **solo el write-side** -- `domain-scaffolder` genera el segundo `.WithTracing(...)` con el `SetSampler` posterior a `UseAzureMonitorExporter()` (#511) y la linea `Durability.DurabilityMetricsEnabled = false` dentro del callback (#512). El seam del worker **no**: `projections-scaffolder` genera `ConfiguracionObservabilidadProjections` deliberadamente **sin ningun sampler** (su regla absoluta 10, MEF-ADR-0034 seccion 10 punto 4) hasta que aterrice #513. Consecuencia operativa: los chequeos de **ausencia** aplican solo al seam del write-side. Que `ConfiguracionObservabilidadProjections` no instale ningun sampler **no es hallazgo** de este gate mientras el agente que lo genera siga prohibiendolo -- flagearlo arrastraria, via tu propio mandato de corregir, una regresion contra el agente vigente. Lo que si es hallazgo en ese archivo, con o sin #513: un diff que **quita o degrada** un sampler que ya estaba (punto 2).
...
2. **Read-side: si el worker instala un sampler, debe seguir envuelto en `ParentBasedSampler`** (MEF-ADR-0038 seccion 5). Si el diff toca `ConfiguracionObservabilidadProjections` **y ese seam ya instala un sampler** (por defecto todavia no lo hace -- frontera de arriba), el sampler que descarta el span de polling del daemon (`marten.daemon.highwatermark`) debe seguir **envuelto en un `ParentBasedSampler`**. Sin ese envoltorio, un `Sampler` plano que devuelve `Drop` para el span raiz del daemon no evita que Marten instancie igual el span hijo de la consulta Npgsql subyacente -- el filtro por nombre elimina el span visible pero no el ruido real, que sigue generandose y facturandose igual. Un diff que reemplaza el `ParentBasedSampler` por un sampler plano (por ejemplo "simplificando" el wiring) o que quita el filtro por nombre es hallazgo bloqueante; que el seam nunca haya tenido sampler, no.
