...
Estos valores van como **defaults versionados** de las variables del entorno. Un `terraform.tfvars` ignorado sirve exclusivamente para overrides locales y no versionados; un runner limpio de CI no lo recibe. En particular, un cambio persistente de `postgresql_location` y `postgresql_region_short` debe editar ambos defaults juntos en `variables.tf` y commitearse cuando PostgreSQL use otra region.
