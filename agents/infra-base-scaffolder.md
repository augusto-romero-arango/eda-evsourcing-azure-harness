...
**La imagen de este Container App la gobierna CI, no Terraform (issue #456).** `var.image` (alimentada por `var.projections_worker_image` en el esqueleto del entorno, Paso 2.3b) solo determina la imagen del **bootstrap**: el placeholder publico con el que Terraform crea el recurso la primera vez. A partir de ahi, el pipeline de CI que construye y publica la imagen real del worker (issue hermano, fuera de alcance de este agente) es su unico dueno -- la actualiza en Azure directamente con `az containerapp update --image`, sin tocar el HCL ni la variable. Por eso el recurso declara `lifecycle { ignore_changes = [template[0].container[0].image] }` (ver abajo): sin ese bloque, cada `terraform plan` posterior detectaria la imagen real como una desviacion del estado deseado (`var.projections_worker_image`, todavia en el placeholder) y propondria revertirla. Respaldo documental (HashiCorp, Terraform docs, *The `lifecycle` Meta-Argument* -- seccion `ignore_changes`): *"To let Terraform share management responsibilities of a single object with a separate process, the `ignore_changes` meta-argument specifies resource attributes that Terraform should ignore when planning updates to the associated remote object. Terraform considers the arguments corresponding to the given attribute names when planning a create operation, but are ignored when planning an update operation."* Por esa misma cita, `var.image`/`var.projections_worker_image` y su default placeholder siguen sirviendo intactos para el `create` del bootstrap -- `ignore_changes` solo entra en juego al planificar un `update`.

**Restriccion de alcance de ese `ignore_changes`.** El meta-argumento `lifecycle` no admite expresiones dinamicas ni condicionales -- no puede leerse de una variable ni activarse solo para un `name` en particular. `ignore_changes = [template[0].container[0].image]` aplica entonces a **cualquier** Container App que este modulo instancie en el futuro, no solo al worker de proyecciones. Hoy el modulo tiene un unico consumidor (el worker del entorno, Paso 2.3b) y esa semantica -- imagen gobernada por un proceso externo a Terraform -- es exactamente la suya. Quien necesite instanciar un Container App con otra semantica (imagen gobernada por Terraform) tendra que separar ese caso en un modulo distinto en vez de reusar este.
