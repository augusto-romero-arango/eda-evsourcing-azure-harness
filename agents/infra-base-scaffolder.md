...
- `postgresql_region_short` -- por defecto, el mismo token `azureRegionShort`; alimenta **solo** el componente `{region}` del nombre de PostgreSQL. Si `postgresql_location` difiere de la region primaria por una restriccion de oferta, el consumidor debe declarar tambien su abreviatura real (p. ej. `centralus`/`cus`) en `terraform.tfvars`. No la derives de `postgresql_location`: MEF-ADR-0045 no admite una tabla interna de abreviaturas regionales. Este override no altera `local.prefix` ni `local.prefix_func`, que conservan la region primaria para los demas recursos.
...
Estos valores van como **defaults** de las variables del entorno; el consumidor los sobreescribe via `terraform.tfvars`. En particular, `postgresql_location` y `postgresql_region_short` son overrides no sensibles que deben cambiarse juntos cuando PostgreSQL use otra region.
...
variable "postgresql_location" {
  description = "Region ARM del servidor PostgreSQL Flexible. Si difiere de la primaria, ajusta tambien postgresql_region_short con la abreviatura declarada por el consumidor."
  type        = string
  default     = "<location>"
}

variable "postgresql_region_short" {
  description = "Abreviatura de la region real de PostgreSQL (componente {region} CAF). Default: azureRegionShort; no se deriva de postgresql_location."
  type        = string
