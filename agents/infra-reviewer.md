...
- **Worker de proyecciones sin `Microsoft.App` registrado (MEF-ADR-0034 seccion 8): si `infra/environments/<env>/main.tf` instancia el modulo `container-app-environment` o `container-app` (`module "container_app_environment"`/`module "container_app"`), el bloque `provider "azurerm"` de `infra/environments/<env>/providers.tf` del mismo entorno debe declarar el argumento `resource_providers_to_register` con `"Microsoft.App"` incluido en la lista.** Mira ese argumento puntual, no el modo `resource_provider_registrations` (`core`/`extended`/`all`/`none`/`legacy`): `Microsoft.App` no aparece en ninguno de los cinco sets del provider `azurerm` v4 (verificado contra `internal/resourceproviders/required.go` del propio provider), asi que subir el modo a `extended` o `all` no lo registra -- aceptar eso como equivalente es un falso negativo. Si `resource_providers_to_register` falta, o existe sin `"Microsoft.App"` en la lista, es un **hallazgo de arquitectura**: sin el, el primer `apply` que crea el `container-app-environment` falla con `409 MissingSubscriptionRegistration`, con el `apply` a medias (identidad, role assignments y `container-registry` ya creados, `container-app-environment`/`container-app` sin intentarse) pero el state consistente. Corrigelo agregando (o sumando a la lista existente) `resource_providers_to_register = ["Microsoft.App"]` dentro del bloque `provider "azurerm"` -- **nunca** propongas el recurso `azurerm_resource_provider_registration` como correccion: su `destroy` intenta desregistrar el namespace y falla si quedan recursos de ese namespace en la suscripcion, y su state -- a nivel de suscripcion, no de entorno -- quedaria compartido entre todos los entornos que la comparten (MEF-ADR-0034 seccion 8). Si el entorno no instancia ninguno de los dos modulos, este chequeo no aplica -- no emitas hallazgo.
