...
jobs:
  ...
  deploy:
    ...
    # que un cambio futuro de paths o un workflow_dispatch accidental exponga Azure.
...
> **Repos ya scaffoldeados antes del fix del issue #1248:** `deploy-{kebab}.yml` tampoco se sobrescribe por idempotencia. En cada workflow existente, agrega `pull_request.paths` con las mismas seis rutas de `push.paths` (`src/<RootNamespace>.{PascalCase}/**`, `src/<RootNamespace>.{PascalCase}.DomainEvents/**`, `src/<RootNamespace>.PublicEvents/**`, `src/<RootNamespace>.PrivateEvents/**`, `global.json` y el propio `.github/workflows/deploy-{kebab}.yml`); conserva `workflow_run` y `workflow_dispatch`. Luego cambia las condiciones de los jobs `deploy` y `smoke-tests` a `github.event_name != 'pull_request' && needs.determinar-alcance.outputs.debe_desplegar == 'true'` (en `smoke-tests`, agrega tambien `determinar-alcance` a `needs`). No dupliques `build-and-test`: el job existente es el unico que debe restaurar, compilar Release y recorrer `tests/<RootNamespace>.*.Tests/`; el glob ya excluye `.SmokeTests/`.
