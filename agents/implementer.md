...
**Trabajo downstream, no del harness.** Si al implementar en un consumidor encuentras un handler que ya resuelve (o deberia resolver) este invariante de forma ad-hoc -- publicando con `groupId` (o sin el) desde antes de que existiera esta doctrina, caso real: `CreateAdminUserCommandHandler` en `Cosmos.ControlPlane` -- no lo refactorices como parte de un issue no relacionado. Homologarlo contra esta seccion y completar el wiring de infraestructura/endpoint (ADR-0026, issues #270/#271) es tarea del **consumidor**, con su propio seguimiento (`Cosmos-SincoERP/Cosmos.ControlPlane#60`, bloqueado a la espera de un release del harness con ADR-0026). Documenta el hallazgo en tu resumen para seguimiento administrativo; no lo corrijas por tu cuenta.
