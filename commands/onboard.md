...
2. **Directivas canónicas del consumidor** (MEF-ADR-0049/0050/0053): comprueba independientemente que `AGENTS.md` tenga las secciones obligatorias "Tokens del harness" y "Verificación de fuentes", incluidos los 5 tokens (`RootNamespace`, `SolutionFile`, `ProjectDisplayName`, `BoundedContext`, `BoundedContextDomains`), y que `CLAUDE.md` contenga una línea independiente exacta `@AGENTS.md`. `AGENTS.md` es la fuente canónica para todos los runtimes; `CLAUDE.md` es el puente explícito que Claude Code requiere. Si conserva cualquiera de las dos secciones contractuales, reporta `FALTA` y pide limpiarlas manualmente, sin tocar el archivo. Un `CLAUDE.md` legacy legible sigue visible como fallback de migración, pero nunca deja el proyecto canónicamente listo. Un archivo existente no legible reporta `NO VERIFICADO`.
...
Al cerrar el reporte, `/onboard` imprime un bloque **"Proximos pasos"**: deriva del mismo diagnostico ya hecho (sin re-verificar nada) la migración de directivas, siempre en este orden: completar `AGENTS.md`, dejar el puente `CLAUDE.md` -> `@AGENTS.md`, y limpiar manualmente las secciones contractuales legacy duplicadas; después enumera labels, CI, infraestructura, secretos, auth y proyecciones según corresponda. Es **puramente informativo**: no ejecuta `gh`/`az`, no provisiona ni copia, borra o reescribe doctrina.
