...
**Caveat: idempotencia parcial de la condicion ABAC.** Si el repo ya tenia la aplicacion de Entra del CI provisionada por una version de `setup-github-ci.sh` **anterior al issue #195** (sin la condicion anti-escalacion en `Role Based Access Control Administrator`), re-correr el script aqui **no** le aplica la condicion a la asignacion ya existente (`az role assignment create` no actualiza condiciones sobre una asignacion previa con el mismo rol/alcance/principal). El SP quedaria con ese rol sin la restriccion anti-escalacion, aunque `/onboard` reporte el CI en verde. Ver el diagnostico y la remediacion completa en el README, seccion "Bootstrap de infraestructura", paso 2. El arreglo del script (deteccion y recreacion automatica) queda diferido a un issue aparte.
