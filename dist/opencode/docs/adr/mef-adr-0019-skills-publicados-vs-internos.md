...
La regla aplica por igual a los dos lados, aunque el gate quede fuera del alcance del PR por caminos distintos. Del lado interno, la allowlist (`is_path_in_mefisto_scope`, `.claude/scripts/_mefisto-common.sh`) se carga desde el checkout principal del repo -- que esta en `main` -- porque el pipeline se invoca ahi y crea el worktree como directorio aparte. Del lado publicado, el pipeline corre desde el plugin instalado (`commands/tooling.md` resuelve `PLUGIN_ROOT` bajo `~/.claude/plugins/cache/`) y lee su blocklist (`is_path_in_consumer_blocklist`, `scripts/_pipeline-common.sh`) de esa copia, nunca del repo del consumidor: registrar una ruta nueva ahi es un PR de este repo, y solo alcanza al consumidor con la siguiente version del plugin. En los dos casos vale la misma invariante -- **el PR bajo revision no puede alterar el gate que lo juzga** -- y de ella se deriva la regla.
