...
```bash
MEFISTO_RUNTIME=opencode "${MEFISTO_PACKAGE_ROOT}/scripts/tmux-pipeline.sh" $ARGUMENTS
```

Con un unico scaffold confirmado, ejecuta la forma equivalente agregando el dominio kebab-case confirmado:

```bash
MEFISTO_RUNTIME=opencode "${MEFISTO_PACKAGE_ROOT}/scripts/tmux-pipeline.sh" $ARGUMENTS --scaffold-domain <dominio-kebab-confirmado>
```
