...
| `always_on` | `true` | Default unico del marco, sin distincion dev/prod. En los tiers de computo dedicado que exige este ADR (Basic o superior; Y1 proscrito) el cargo es por **instancia de VM del plan** -- identico sin importar cuantas apps corran en ella ni si estan atendiendo trafico --, y Always On no figura entre las tres unicas features que la doc enumera como cargo adicional (dominios de App Service, certificados de App Service y conexiones TLS basadas en IP) [7]: apagarlo **no ahorra**. La doc de Azure Functions es explicita en el otro sentido: en un App Service plan hay que habilitar Always On "para que la function app funcione correctamente", porque el runtime de Functions se queda idle tras unos minutos de inactividad [8] -- y ese idle interrumpe el poll del outbox de Wolverine (`DurabilityMode.Solo`, ver Contexto). |
...
- **[7]** "What are Azure App Service plans? - Cost of App Service plans" -- en los tiers de computo dedicado (Basic, Standard, Premium, PremiumVx) se cobra **cada instancia de VM** del plan, y esas instancias "are charged the same, regardless of how many apps are running on them"; del uso de las features de App Service no se cobra nada, con tres excepciones enumeradas (App Service domains, App Service certificates y conexiones TLS basadas en IP) entre las que Always On no aparece. https://learn.microsoft.com/azure/app-service/overview-hosting-plans#cost-of-app-service-plans
- **[8]** "Dedicated hosting plans for Azure Functions - Always On" -- "When you run your app on an App Service plan, you should enable the **Always on** setting so that your function app runs correctly": en un App Service plan el runtime de Functions se queda idle tras unos minutos de inactividad, y el ajuste Always On solo existe en ese tipo de plan (en los demas la plataforma activa la app automaticamente). https://learn.microsoft.com/azure/azure-functions/dedicated-plan#always-on
...
- 2026-08-17: enmendado (issue #652) para corregir el default de `always_on` a `true` unico en todo el marco (sin distincion dev/prod): el fundamento de "ahorro en dev" era factualmente falso en los tiers de computo dedicado que este mismo ADR exige (Basic o superior; Y1 proscrito), donde se cobra cada instancia de VM del plan sin importar la actividad de las apps que corren en ella y Always On no esta entre las tres features que generan cargo adicional [7], y la doc oficial de Azure Functions indica habilitarlo en planes dedicados para que la function app funcione correctamente [8]. Se elimina del cuerpo toda afirmacion de que `false` ahorra costo. Se fija ademas el mecanismo de wiring que cierra el output huerfano documentado en MEF-ADR-0021: el modulo `function-app` gana un input `always_on` aplicado en `site_config.always_on`, y el `domain-scaffolder` lo conecta pasando el output del `service-plan`.
