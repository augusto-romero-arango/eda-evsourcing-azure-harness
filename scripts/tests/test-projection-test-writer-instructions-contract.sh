#!/usr/bin/env bash
# test-projection-test-writer-instructions-contract.sh -- Neutraliza la
# lectura del contrato consumidor en projection-test-writer (#1428,
...
SOURCE_AGENT="$REPO_ROOT/src/published/agents/projection-test-writer.md"
CLAUDE_AGENT="$REPO_ROOT/agents/projection-test-writer.md"
...
OPENCODE_AGENT="$WORK/opencode-projection-test-writer.md"
...
if [ -s "$OPENCODE_AGENT" ]; then pass "el adaptador OpenCode renderiza projection-test-writer"; else fail "el adaptador OpenCode no genero salida: $(cat "$WORK/opencode.err")"; fi
