...
OUT=$(run "$R" --apply); RC=$?
if [ "$RC" -eq 0 ] && grep -Fq '**RootNamespace**: Empresa.Proyecto' "$R/AGENTS.md" && grep -Fq '**SolutionFile**: Proyecto.slnx' "$R/AGENTS.md" && grep -Fq '**ProjectDisplayName**: Proyecto' "$R/AGENTS.md" && grep -Fq '**BoundedContext**: Principal' "$R/AGENTS.md" && grep -Fq '**BoundedContextDomains**: ventas, cobros' "$R/AGENTS.md" && grep -Fq 'Verificación de fuentes' "$R/AGENTS.md" && grep -Fxq '@AGENTS.md' "$R/CLAUDE.md"; then pass "apply greenfield deriva los cinco tokens y crea el puente"; else fail "apply greenfield falló: $OUT; AGENTS=$(cat "$R/AGENTS.md" 2>/dev/null); CLAUDE=$(cat "$R/CLAUDE.md" 2>/dev/null)"; fi
if grep -Fq 'Ofrece este paso **solo** cuando el diagnóstico reportó que falta `AGENTS.md` o el puente exacto' "$COMMAND" && grep -Fq 'Sin un `si` explícito, no ejecutes ninguna escritura' "$COMMAND" && grep -Fq 'MIGRATE_SCRIPT="${PLUGIN_SCRIPTS%/}/onboard-migrate-directives.sh"' "$COMMAND"; then pass "el comando resuelve el script y condiciona la aplicación a diagnóstico y confirmación explícita"; else fail "el comando no conserva el gate humano requerido"; fi
...
EOF
AGENTS_BEFORE=$(cksum "$R/AGENTS.md"); printf '# Directiva propia\nNo perder.\n' > "$R/CLAUDE.md"; run "$R" --apply >/dev/null
if [ "$AGENTS_BEFORE" = "$(cksum "$R/AGENTS.md")" ] && grep -Fq 'No perder.' "$R/CLAUDE.md" && grep -Fxq '@AGENTS.md' "$R/CLAUDE.md"; then pass "AGENTS completo se conserva y CLAUDE.md propio recibe import"; else fail "se sobrescribió AGENTS o no se preservó CLAUDE"; fi
...
OUT=$(run "$R" --preview); RC=$?
if [ "$RC" -ne 0 ] && [ "$BEFORE" = "$(cksum "$R/AGENTS.md")" ] && [ ! -e "$R/CLAUDE.md" ] && printf '%s' "$OUT" | grep -Fq 'Complétalo manualmente'; then pass "preview incompleto rechaza con instrucción manual y sin escrituras"; else fail "preview incompleto no fue conservador: $OUT"; fi
