#!/usr/bin/env bash
# test-implementer-instructions-contract.sh -- Neutraliza la lectura del
# contrato consumidor en implementer (#1430, MEF-ADR-0053 seccion 4).
#
# Demuestra que la resolucion de RootNamespace en las salidas generadas
# Claude y OpenCode del stage verde write-side procede exclusivamente del
...
SOURCE_AGENT="$REPO_ROOT/src/published/agents/implementer.md"
CLAUDE_AGENT="$REPO_ROOT/agents/implementer.md"
...
OPENCODE_AGENT="$WORK/opencode-implementer.md"
...
if [ -s "$OPENCODE_AGENT" ]; then pass "el adaptador OpenCode renderiza implementer"; else fail "el adaptador OpenCode no genero salida: $(cat "$WORK/opencode.err")"; fi
