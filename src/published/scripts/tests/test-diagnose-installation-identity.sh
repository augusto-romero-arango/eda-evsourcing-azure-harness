...
mkdir -p "$OPENCODE"
OUT="$(run_diagnostic "$CLAUDE" "$OPENCODE")"; assert_status "$OUT" metadata_unavailable 'metadata ausente se distingue de una instalacion ausente'
...
OUT="$(run_diagnostic "$CLAUDE" "$OPENCODE")"; assert_status "$OUT" metadata_unavailable 'metadata corrupta se distingue de deriva'
