...
jq -e '.bash["*"] == "deny" and .bash["${MEFISTO_PACKAGE_ROOT}/scripts/*"] == "allow" and .bash["dotnet *"] == "allow" and .bash["func init *"] == "allow" and .bash["terraform init -backend=false*"] == "allow" and .bash["terraform validate*"] == "allow" and .bash["terraform fmt*"] == "allow" and .bash["python3 - *"] == "allow" and .bash["python3 -m json.tool*"] == "allow" and .bash["cd *"] == "allow" and .bash["echo *"] == "allow" and .bash["test *"] == "allow" and .bash["[ *"] == "allow" and .bash["touch *"] == "allow" and .bash["tr *"] == "allow" and .bash["mv *"] == "allow" and .bash["ilspycmd *"] == "allow" and .bash["rm *"] == "deny" and .bash["rm -f src/*"] == "allow" and .bash["rm -rf src/*"] == "allow" and .bash["rm -f tests/*"] == "allow" and .bash["rm -f \"src/*"] == "allow" and .bash["rm -rf \"src/*"] == "allow" and .bash["rm -f \"tests/*"] == "allow" and ((.bash | to_entries | map(.key)) as $rules | ($rules | index("rm *")) < ($rules | index("rm -f src/*"))) and .external_directory == "deny"' <<< "$shell_permission" >/dev/null && pass 'combinacion shell acotada' || fail 'combinacion shell acotada'
