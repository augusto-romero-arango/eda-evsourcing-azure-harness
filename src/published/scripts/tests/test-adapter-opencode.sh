...
jq -e --argjson count "$skill_file_count" 'length == ($count + 2) and ([.[] | select(.source == "skills/projections/SKILL.md" and .destination == "skills/mefisto-projections/SKILL.md")] | length) == 1 and ([.[] | select(.id == "interactive-observability" and .source == "src/published/hooks/interactive-hooks.json" and .destination == "plugins/mefisto-observability.js" and .mode == "0644")] | length) == 1 and ([.[] | select(.id == "mefisto-manifest" and .destination == "mefisto-manifest.json")] | length) == 1' <<< "$assets" >/dev/null && pass 'assets preservan Skills y observabilidad interactiva' || fail 'inventario de assets incompleto'
