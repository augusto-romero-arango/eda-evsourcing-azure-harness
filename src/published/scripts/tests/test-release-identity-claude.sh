...
bash "$GENERATOR" --out "$OUT" "$REPO_ROOT/src/published/agents/tooling-reviewer.md" >/dev/null
...
check="$(bash "$GENERATOR" --check --out "$OUT" "$REPO_ROOT/src/published/agents/tooling-reviewer.md")"; rc=$?
