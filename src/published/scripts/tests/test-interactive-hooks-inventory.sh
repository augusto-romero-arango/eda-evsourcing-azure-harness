#!/usr/bin/env bash
# Demuestra la trazabilidad de los siete comandos legacy y los hechos neutrales.
set -uo pipefail
...
[ "$legacy_count" = "7" ] && [ "$contract_count" = "7" ] && pass "el inventario legacy tiene siete comportamientos y el contrato siete hechos" || fail "conteos legacy=$legacy_count contrato=$contract_count"
