...
for index in "${!agents[@]}"; do
    ...
    if [ "$agent" = test-writer ]; then pass "$agent incorpora el preambulo de package root al proyectar Claude"; elif diff -u <(translated_source_body claude "$source") <(body_without_adapter_lines "$mirror") >/dev/null; then pass "$agent conserva el cuerpo traducido al proyectar Claude"; else fail "$agent altera el cuerpo al proyectar Claude"; fi
...
for agent in "${agents[@]}"; do
    ...
    if [ "$agent" = test-writer ]; then pass "$agent incorpora el preambulo de package root al proyectar OpenCode"; elif diff -u <(translated_source_body opencode "$REPO_ROOT/src/published/agents/$agent.md") <(body_without_adapter_lines "$opencode") >/dev/null; then pass "$agent conserva el cuerpo traducido al proyectar OpenCode"; else fail "$agent altera el cuerpo al proyectar OpenCode"; fi
