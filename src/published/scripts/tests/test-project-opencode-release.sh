...
make_release() {
    ...
    mkdir -p "$root/commands" "$root/agents" "$root/plugins" "$root/skills/mefisto-prueba"
    printf 'comando %s\n' "$version" > "$root/commands/mefisto:tooling.md"
    printf 'agente %s\n' "$version" > "$root/agents/mefisto-writer.md"
    printf 'plugin %s\n' "$version" > "$root/plugins/mefisto.js"
    printf '%s\n' '---' 'name: mefisto-prueba' 'description: Prueba.' '---' > "$root/skills/mefisto-prueba/SKILL.md"
...
rm -f "$XDG_DATA_HOME/mefisto/releases/2.0.0/plugins/mefisto.js" "$XDG_DATA_HOME/mefisto/releases/2.0.0/skills/mefisto-prueba/SKILL.md"
...
[ -L "$XDG_CONFIG_HOME/opencode/commands/mefisto:tooling.md" ] && [ -L "$XDG_CONFIG_HOME/opencode/agents/mefisto-writer.md" ] && [ -L "$XDG_CONFIG_HOME/opencode/skills/mefisto-prueba/SKILL.md" ] && pass 'proyecta el layout global documentado' || fail 'faltan enlaces globales'
...
[ ! -e "$XDG_CONFIG_HOME/opencode/plugins/mefisto.js" ] && [ ! -e "$XDG_CONFIG_HOME/opencode/skills/mefisto-prueba/SKILL.md" ] && pass 'retira residuos de capacidades ausentes en la nueva release' || fail 'dejo residuos de la release anterior'
