...
run_fixture "$DIR" "$SESSION_CMD" session.json && jq -s -e '.[1].record_type == "session.started" and .[1].source == "clear" and .[1].model == null and .[1].harness_version == null and .[1].harness_commit == null' "$DIR/.mefisto/pipeline/sessions.jsonl" >/dev/null && pass "clear sin modelo y manifiesto ausente degradan a null" || fail "degradacion de inicio invalida"
...
if (cd "$DIR" && /bin/sh -c "$MODEL_CMD" <<< "$payload_empty") \
    && (cd "$DIR" && /bin/sh -c "$MODEL_CMD" <<< "$payload_missing") \
    && (cd "$DIR" && /bin/sh -c "$MODEL_CMD" <<< "$payload_malformed"); then
