...
case "$1" in
    ...
    assets)
        case "${FIXTURE_ASSETS:-normal}" in
            ...
            destinos-anidados) printf '%s\n' '[{"id":"uno","source":"src/published/assets/config.txt","destination":"repetido","mode":"0644"},{"id":"dos","source":"src/published/assets/launcher.txt","destination":"repetido/hijo","mode":"0644"}]' ;;
            inventario) printf '%s\n' '[{"id":"inventario","source":"src/published/assets/config.txt","destination":".mefisto-generated-assets.json","mode":"0644"}]' ;;
            inseguro) printf '%s\n' '[{"id":"inseguro","source":"../secret","destination":"x","mode":"0644"}]' ;;
            id-inseguro) printf '%s\n' '[{"id":"../inseguro","source":"src/published/assets/config.txt","destination":"x","mode":"0644"}]' ;;
            destino-inseguro) printf '%s\n' '[{"id":"inseguro","source":"src/published/assets/config.txt","destination":"../x","mode":"0644"}]' ;;
            ausente) printf '%s\n' '[{"id":"ausente","source":"src/published/assets/no-existe","destination":"x","mode":"0644"}]' ;;
            symlink) printf '%s\n' '[{"id":"symlink","source":"src/published/assets/link","destination":"x","mode":"0644"}]' ;;
            modo) printf '%s\n' '[{"id":"modo","source":"src/published/assets/config.txt","destination":"x","mode":"0600"}]' ;;
            invalido) printf '%s\n' '{no-json}' ;;
            fallar) printf '%s\n' 'adapter-assets.sh: fallo al enumerar assets' >&2; exit 1 ;;
        esac ;;
    render-asset)
        case "${FIXTURE_ASSETS:-normal}:$2" in render-fallar:*) printf '%s\n' 'adapter-assets.sh: fallo al renderizar asset' >&2; exit 1 ;; *) printf 'renderizado:%s\n' "$(cat "$3")" ;; esac ;;
    *) exit 1 ;;
esac
