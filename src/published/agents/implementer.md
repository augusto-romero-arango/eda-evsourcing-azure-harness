...
- Reporta el gap al llamador del pipeline (escribe en `{{mefisto:state-path blockage-report.md}}` seccion "Issue incompleto: falta ADRs aplicables") y termina normalmente.
...
3. **Escribe el reporte de bloqueo** en `{{mefisto:state-path blockage-report.md}}`.
...
Crea el archivo `{{mefisto:state-path summaries/stage-2-implementer.md}}`:
