...
Los ADRs del harness viven **dentro de la release activa e inmutable del plugin**, no en el repo donde corres este agente (`cwd = repo consumidor`). La raiz canonica es `{{mefisto:package-root}}`: abre cada ADR en `{{mefisto:package-root}}/docs/adr/<archivo>.md`. **Nunca uses la ruta relativa `docs/adr/...`**: con `cwd = repo consumidor` resolveria contra `<consumer>/docs/adr/...` (inexistente) y el ADR pareceria "ausente".

Los recursos de Nivel 3 del Skill `projections` se abren desde su raiz logica adaptada: `{{mefisto:skill-root projections}}/modelos-marten.md`, `{{mefisto:skill-root projections}}/read-apis.md`, `{{mefisto:skill-root projections}}/naming.md` y `{{mefisto:skill-root projections}}/config-test.md`. No copies esa doctrina al agente.
...
4. Si despues de 5 intentos no lo resuelves:
   ...
   - **Actualiza el reporte** `{{mefisto:state-path blockage-report.md}}` agregando tu seccion:
...
3. **GET de read-side que reenvia el segmento de ruta sin parsear.** Autoridad: MEF-ADR-0037 seccion 2. **Lo que se verifica es el parseo, no el tipo declarado en la firma**: el binding del `HttpTrigger` recibe el segmento de ruta como `string`, y esa **es** la forma canonica del marco (el agente logico `projection-implementer`, `{{mefisto:skill-root projections}}/read-apis.md`; el enlace tipado en el worker aislado esta declarado *no verificado* en `{{mefisto:skill-root projections}}/naming.md`). Una firma `string id` no es, por si sola, un hallazgo -- no la "corrijas" a `Guid id`. El hallazgo bloqueante es que ese valor viaje **sin parseo** hasta `LoadAsync`/`AggregateStreamAsync`/`FetchStreamAsync`/`ExistsAsync`.
   ...
   **Criterio operativo para clasificar la identidad antes de flagear**: lee la definicion de identidad del aggregate en el codigo, no la infieras del segmento de ruta. Si el aggregate declara `ComputarStreamId(...)` con varios componentes, es la forma (c) de abajo; si su id es un unico `Guid` o un VO de un solo componente, es la forma (a) o (b).
   ...
   El route constraint `{id:guid}` **no** sustituye el parseo -- produce `404` en vez de `400` y no normaliza el casing (MEF-ADR-0037 seccion 2 y Alt 4); declararlo por su proposito real (desambiguar rutas parecidas) sigue siendo legitimo, pero el parseo con `400` va igual. Corrige siguiendo el ejemplo canonico de `{{mefisto:skill-root projections}}/read-apis.md`.
...
- Nombre segun MEF-ADR-0016: `<Sujeto>_<LoQuePasa>[_Cuando<Condicion>]`. Para command handlers el sujeto es el nombre del comando (`RegistrarMarcacion`, `CrearTurno`), nunca `Debe...` ni `HandleAsync`. Ver `{{mefisto:package-root}}/docs/adr/mef-adr-0016-convencion-naming-tests.md`.
...
Crea el archivo `{{mefisto:state-path summaries/stage-3-reviewer.md}}` con el siguiente formato:
