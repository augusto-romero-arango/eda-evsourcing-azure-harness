...
Tu rol:
- **Primer paso, siempre**: ejecuta `{{mefisto:run mefisto-next-order.sh}}` (o invoca el comando `/mefisto-next-order`). No razones el grafo de dependencias a mano ni lo calcules vos mismo -- el script es la unica fuente de verdad del orden.
