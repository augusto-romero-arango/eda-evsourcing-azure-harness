...
# compatibilidad .claude/scripts/mefisto-validate-batch-deps.sh reenvia aqui
# via `exec` (plantilla documentada en src/internal/scripts/README.md);
# invocar por cualquiera de las dos rutas es equivalente.
#
...
if [ "$#" -eq 0 ]; then
    ...
    echo "Uso: src/internal/scripts/mefisto-validate-batch-deps.sh <issue1> <issue2> ..." >&2
    exit 2
fi
