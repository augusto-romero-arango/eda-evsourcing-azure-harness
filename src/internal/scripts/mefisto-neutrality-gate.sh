...
#
# Uso: mefisto-neutrality-gate.sh [--root <dir>]
...
#        --check` (sobre el mismo --root) no reporta divergencias.
#
...
# se lee SIEMPRE relativa a --root (no a este script): es el propio --root
# quien declara sus excepciones vigentes -- un PR que registra una excepcion
# nueva y uno que la usa pueden ser el mismo worktree sin que el gate mire una
# copia desactualizada en el checkout principal. Toda entrada exige `motivo`
# no vacio (CA-1); si falta alguno, el gate aborta antes de escanear nada.
...
while [ $# -gt 0 ]; do
    case "$1" in
        --root)
            ...
            ;;
        *)
            echo "ERROR: argumento desconocido: $1 (uso: mefisto-neutrality-gate.sh [--root <dir>])" >&2
            exit 1
            ;;
    esac
done
...
ALLOWLIST_FILE="$ROOT/src/internal/contract/neutrality-allowlist.json"
...
#
# CA-3: un unico `grep -nHE` sobre todo <archivo...> (nunca un grep por
# archivo). Cada coincidencia se filtra contra get_allowlist_patterns DESPUES
# -- ese filtro corre sobre las (pocas) lineas ya encontradas, no sobre el
# universo de archivos completo.
run_text_rule() {
    ...
    local raw
    raw="$(grep -nHE "$regex" -- "${rule_files[@]}" 2>/dev/null)" || true
    [ -z "$raw" ] && return 0
...
R2_REGEX='claude -p|claude --agent|opencode run'
...
if [ -f "$ADAPTERS_SCRIPT" ]; then
    ADAPTERS_OUT="$(bash "$ADAPTERS_SCRIPT" --check 2>&1)" || true
    if [ -n "$ADAPTERS_OUT" ]; then
