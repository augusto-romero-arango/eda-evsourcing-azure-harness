...
```bash
TOOLING_SKILL="$(git rev-parse --show-toplevel)/{{mefisto:command-path mefisto-tooling}}"
[ -f "$TOOLING_SKILL" ] || {
...
Si el bloque imprime `ERROR`, detente sin lanzar nada.
