...
(en el mismo orden de `$ARGUMENTS`). Invoca el validador del batch pasando esa lista como
argumentos, respetando el orden del batch:

```bash
{{mefisto:run mefisto-validate-batch-deps.sh <issue1> <issue2> ...}}
```
