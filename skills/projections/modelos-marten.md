...
Los tres primeros, verificados por reflexion propia (Marten `9.12.0`, SDK .NET 10.0.201, issue #495): `typeof(Marten.Events.Aggregation.SingleStreamProjection<,>).Namespace` y equivalentes (MEF-ADR-0035 ref. [19]); `IEvent`, contra el codigo fuente del paquete (MEF-ADR-0035 ref. [16]). Esta tabla cubre **todos** los `using` de paquete de los ejemplos N1/N2 de abajo -- si un ejemplo necesita un simbolo de Marten/JasperFx que no esta aqui, resuelve su namespace verificando, no por analogia. El unico `using` restante de esos ejemplos es `<RootNamespace>.{Dominio}.DomainEvents`, cuyo namespace lo fija el marco y no un paquete (parrafo de procedencia de arriba): no se verifica por reflexion, se resuelve leyendo el `CLAUDE.md` del consumidor.
